...
function ReportsPage({students,results,comments,term,setTerm,year,setYear,examType,setExamType,logo,users,ttSetup}) {
  ...
  function buildClassResultsHTML(classStudents, className) {
    ...
    const rows=ranked.map((s,i)=>{
      ...
      return `<tr style="background:${i%2===0?"rgba(255,255,255,0.55)":"rgba(248,250,252,0.55)"}">
        ...
        <td style="padding:6px 8px;font-weight:bold;white-space:nowrap;min-width:200px;max-width:220px;overflow:hidden;text-overflow:ellipsis;vertical-align:${i%2===0?"top":"bottom"};">${(s.name||"").trim()}</td>
        ${subs.map(su=>{const isComb=su===SCI_AGRI_COMBINED;const v=getPrintMark(sr,su);return`<td style="padding:5px 4px;text-align:center;font-weight:bold;${isComb?`background:${SCI_AGRI_BG};color:${SCI_AGRI_DARK};`:""}">${v!==null?(isComb?v.toFixed(1):v):"—"}</td>`;}).join("")}
    ...
    return `<div style="margin-bottom:32px;">
      ...
      <table class="results-table" style="width:100%;border-collapse:collapse;font-size:10px;min-width:600px;">
        <thead><tr style="background:#3b0764;">
          ...
          ${subs.map(s=>{const isComb=s===SCI_AGRI_COMBINED;const short=getSubShort(s);return`<th style="padding:0;width:42px;min-width:42px;height:85px;vertical-align:bottom;background:${isComb?"#0d9488":"#3b0764"};position:relative;overflow:visible;" title="${s}"><div style="position:absolute;bottom:10px;right:4px;transform:rotate(-45deg);transform-origin:right bottom;white-space:nowrap;font-size:11px;font-weight:bold;color:white;">${short}</div></th>`;}).join("")}
  ...
  function buildClassGradesHTML(classStudents, className) {
    ...
    const teacherRow=`<tr style="background:#f5f3ff;border-top:2px solid #1d4ed8;">
      ...
      ${subs.map(s=>{const isComb=s===SCI_AGRI_COMBINED;const ts=isComb?[...new Set(["Integrated Science","Agriculture and Nutrition"].flatMap(p=>getSubjectTeachersForResult(className,p)).filter(Boolean))]:getSubjectTeachersForResult(className,s);const name=ts.length?ts.join(" / "):"—";return`<td style="padding:4px 3px;text-align:center;font-size:7.5px;color:${isComb?SCI_AGRI_DARK:"#1d4ed8"};font-weight:bold;white-space:normal;word-break:break-word;max-width:60px;">${name}</td>`;}).join("")}
      <td style="padding:4px 6px;font-size:9px;color:#64748b;"></td>
    </tr>`;
    return `<div style="margin-bottom:32px;">
      ${buildSectionHeader(logo)}
      ...
      <table class="grade-table" style="width:100%;border-collapse:collapse;font-size:10px;min-width:600px;">
        <thead><tr style="background:#1d4ed8;">
          <th style="padding:5px 6px;text-align:center;color:white;">Pos</th>
          <th style="padding:5px 8px;text-align:left;color:white;min-width:220px;">Name</th>
          ${subs.map(s=>{const isComb=s===SCI_AGRI_COMBINED;const short=getSubShort(s);return`<th style="padding:0;width:42px;min-width:42px;height:85px;vertical-align:bottom;background:${isComb?"#0d9488":"#1d4ed8"};position:relative;overflow:visible;" title="${s}"><div style="position:absolute;bottom:10px;right:4px;transform:rotate(-45deg);transform-origin:right bottom;white-space:nowrap;font-size:11px;font-weight:bold;color:white;">${short}</div></th>`;}).join("")}
