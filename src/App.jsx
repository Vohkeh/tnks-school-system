...
function ResultsPage({students,results,setResults,comments,setComments,users,term,setTerm,year,setYear,examType,setExamType,user,ttSetup}) {
  ...
  // ── ROLE-BASED ACCESS: restrict entry to assigned class/subject ─────────────
  ...
  const allowedSubs=isAdmin?subs:subs.filter(s=>myAssignments.includes(`${cls}::${s}`));
  const canEdit=isAdmin||(myAssignments||[]).includes(`${cls}::${curSub}`);
...
function SubjectAllocationPage({ staff, user, students }) {
  ...
  const getSubsForClass = (cls) => getSubs(cls);
