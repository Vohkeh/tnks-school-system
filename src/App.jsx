...
function ResultsPage({students,results,setResults,comments,setComments,users,term,setTerm,year,setYear,examType,setExamType,user,ttSetup}) {
  ...
  // ── ROLE-BASED ACCESS: driven entirely by timetable subjectTeachers map ────
  ...
  // FIX: default to the first subject the teacher is actually assigned to (allowedSubs[0]),
  // not always the first in the master list (subs[0] = "English").
  // Previously curSub could be "English" while the dropdown visually showed a different subject,
  // causing the lock message and canEdit check to run against the wrong subject.
  ...
  return (
    <div style={{padding:24}}>
      ...
      <Card style={{marginBottom:18}}><div style={{display:"flex",gap:12,flexWrap:"wrap",alignItems:"flex-end"}}>
        <Sel label="CLASS" value={cls} onChange={v=>{if(dirty&&!window.confirm("You have unsaved marks. Discard and continue?")) return; setDirty(false); setCls(v); setSub("");}} options={allowedClasses}/>
