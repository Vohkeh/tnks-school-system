...
function buildHTMLDoc(title, bodyHTML, logo) {
  ...
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"/><title>${title}</title><style>
    ...
    *{-webkit-print-color-adjust:exact !important;print-color-adjust:exact !important;color-adjust:exact !important;}
      ...
      tfoot{page-break-inside:avoid !important;}
...
function ReportsPage({students,results,comments,term,setTerm,year,setYear,examType,setExamType,logo,users,ttSetup}) {
  ...
  function buildClassResultsHTML(classStudents, className) {
    ...
    const rows=ranked.map((s,i)=>{
      ...
      return `<tr style="background:${i%2===0?"rgba(255,255,255,0.55)":"rgba(248,250,252,0.55)"}">
        ...
        <td style="padding:5px 6px;font-weight:bold;line-height:1.25;white-space:normal;word-break:break-word;min-width:80px;">${_fn}${_ln?`<br><span style="font-size:10px;font-weight:normal;color:#374151;">${_ln}</span>`:""}</td>
    ...
    return `<div style="margin-bottom:32px;">
      ...
      <table class="results-table" style="width:100%;border-collapse:collapse;font-size:10px;min-width:600px;">
        <thead><tr style="background:#3b0764;">
          ...
          ${subs.map(s=>{const isComb=s===SCI_AGRI_COMBINED;const short=getSubShort(s);return`<th style="padding:4px 3px;text-align:center;max-width:40px;white-space:nowrap;vertical-align:middle;background:${isComb?"#0d9488":"#3b0764"};"><div style="writing-mode:vertical-rl;text-orientation:mixed;transform:rotate(180deg);font-size:9px;font-weight:bold;line-height:1.2;height:55px;display:flex;align-items:center;justify-content:center;color:white;" title="${s}">${short}</div></th>`;}).join("")}
  ...
  function buildClassGradesHTML(classStudents, className) {
    ...
    const rows=ranked.map((s,i)=>{
      ...
      return `<tr style="background:${i%2===0?"rgba(255,255,255,0.55)":"rgba(248,250,252,0.55)"}">
        <td style="padding:5px 6px;font-weight:bold;color:${i<3?"#b45309":"#374151"};text-align:center;">${s.position||"—"}</td>
        <td style="padding:5px 6px;font-weight:bold;line-height:1.25;white-space:normal;word-break:break-word;min-width:80px;">${_fn2}${_ln2?`<br><span style="font-size:10px;font-weight:normal;color:#374151;">${_ln2}</span>`:""}</td>
    ...
    return `<div style="margin-bottom:32px;">
      ...
      <table class="grade-table" style="width:100%;border-collapse:collapse;font-size:10px;min-width:600px;">
        <thead><tr style="background:#1d4ed8;">
          <th style="padding:5px 6px;text-align:center;color:white;">Pos</th>
          <th style="padding:5px 6px;text-align:left;color:white;">Name</th>
          ${subs.map(s=>{const isComb=s===SCI_AGRI_COMBINED;const short=getSubShort(s);return`<th style="padding:4px 3px;text-align:center;max-width:40px;white-space:nowrap;vertical-align:middle;background:${isComb?"#0d9488":"#1d4ed8"};"><div style="writing-mode:vertical-rl;text-orientation:mixed;transform:rotate(180deg);font-size:9px;font-weight:bold;line-height:1.2;height:55px;display:flex;align-items:center;justify-content:center;color:white;" title="${s}">${short}</div></th>`;}).join("")}
