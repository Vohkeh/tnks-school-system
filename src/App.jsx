...
function buildStampBox() {
  return `<div class="stamp-box" style="display:flex;justify-content:flex-end;margin-top:14px;">
...
function buildHTMLDoc(title, bodyHTML, logo) {
  ...
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"/><title>${title}</title><style>
    ...
    *{-webkit-print-color-adjust:exact !important;print-color-adjust:exact !important;color-adjust:exact !important;}
      ...
      /* Each class block: force one page, no internal breaks */
      .class-page-block{
      ...
      /* Individual report cards: force single page */
      div[style*="page-break-after:always"]{
        page-break-after: always !important;
        page-break-inside: avoid !important;
      }
      /* Ensure stamp stays with the table — no break before it */
      .stamp-box{
      ...
      /* Scale each class block to fit exactly one A4-landscape page */
      .class-page-block{
  ...
  <script>
    window.addEventListener('load', function() {
      // A4 landscape printable area: 297mm x 210mm, minus 6mm margins each side
      // = 285mm wide x 198mm tall
      var PAGE_W_MM = 285;
      var PAGE_H_MM = 198;
      var pxPerMm = 96 / 25.4;
      var maxW = PAGE_W_MM * pxPerMm;
      var maxH = PAGE_H_MM * pxPerMm;

      function scaleBlocks() {
        var blocks = document.querySelectorAll('.class-page-block');
        blocks.forEach(function(block) {
          // Reset any previous scale so we measure natural size
          block.style.transform = '';
          block.style.width = '';

          var natW = block.scrollWidth;
          var natH = block.scrollHeight;

          var scaleW = natW > maxW ? maxW / natW : 1;
          var scaleH = natH > maxH ? maxH / natH : 1;
          var scale = Math.min(scaleW, scaleH);

          if (scale < 1) {
            block.style.transformOrigin = 'top left';
            block.style.transform = 'scale(' + scale + ')';
            // Shrink the block's layout footprint so next block starts at correct position
            block.style.width = (100 / scale) + '%';
            block.style.marginBottom = '-' + (natH * (1 - scale)) + 'px';
          }
        });
      }

      window.addEventListener('beforeprint', scaleBlocks);
    });
  </script>
  </head><body>
  ${logoWm}
...
function ReportsPage({students,results,comments,term,setTerm,year,setYear,examType,setExamType,logo,users,ttSetup}) {
  ...
  function buildClassResultsHTML(classStudents, className) {
    ...
    return `<div class="class-page-block" style="margin-bottom:32px;">
      ${buildSectionHeader(logo)}
  ...
  function printClassResults(className, mode){
    const stu=students.filter(s=>s.class===className).sort((a,b)=>{const _p=(x,i)=>{const s=(x||"").split("/");return parseInt(s[1])||parseInt(s[s.length-1])||0;}; const na=_p(a.admNo); const nb=_p(b.admNo); return na-nb;});
    const html=`<div class="class-page-block" style="page-break-after:always;">${mode==="grades"?buildClassGradesHTML(stu,className):buildClassResultsHTML(stu,className)}</div>`;
  ...
  function printSchoolResults(mode){
    const html=ALL_CLASSES.map(c=>{
      const stu=students.filter(s=>s.class===c);
      if(!stu.length) return "";
      return `<div class="class-page-block" style="page-break-after:always;">${mode==="grades"?buildClassGradesHTML(stu,c):buildClassResultsHTML(stu,c)}</div>`;
    }).filter(Boolean).join("");
