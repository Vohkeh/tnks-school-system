...
// ── AI API KEYS ────────────────────────────────────────────────────────────────
const GROQ_API_KEY       = "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"; // Timetable
const OPENROUTER_API_KEY = "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"; // Parent Chat
const GEMINI_API_KEY     = "AIzaSyAllyh5eeCacn6EgAXbh-dYISFpFy6A_4o"; // AI Comments
// ──────────────────────────────────────────────────────────────────────────────

// Helper: Groq — Timetable AI Generate (Llama 3.3 70B — best for structured JSON)
...
const STUDENT_TYPES = ["Day Scholar","Boarder"];
...
const SUBJECT_SHORT = {
  ...
  "Integrated Science & Agriculture": "SCI & AGRI",
...
// Helper: subjectTeachers values can be a string (legacy) or array of names
function getTeachersForKey(subjectMap, key) {
  const val = subjectMap[key];
  if(!val) return [];
  return Array.isArray(val) ? val : [val];
}
// Helper: check if a teacher name appears in a subjectTeachers entry
function teacherInKey(subjectMap, key, name) {
  return getTeachersForKey(subjectMap, key).includes(name);
}
...
function getSubs(cls) { const subs=SUBJECTS_MAP[cg(cls)]||[]; if(cls==="PP1") return subs.filter(s=>s!=="Kiswahili Activities"); return subs; }
function getGrade(m) {
...
function PageH({title,sub,children}) {
  return <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20,flexWrap:"wrap",gap:10}}><div><h2 style={{margin:0,color:"#3b0764",fontSize:22,fontFamily:F}}>{title}</h2>{sub&&<div style={{fontSize:13,color:"#64748b",marginTop:2}}>{sub}</div>}</div>{children&&<div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>{children}</div>}</div>;
}
...
function PhotoUp({value,onChange}) {
  ...
  return <div style={{display:"flex",alignItems:"center",gap:12}}><div style={{width:64,height:64,borderRadius:"50%",background:value?"transparent":"#f5f3ff",border:"2px dashed #c4b5fd",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",overflow:"hidden"}} onClick={()=>ref.current?.click()}>{value?<img src={value} alt="photo" style={{width:"100%",height:"100%",objectFit:"cover"}} />:<span style={{fontSize:24}}>📷</span>}</div><div><Btn onClick={()=>ref.current?.click()} v="ghost" style={{fontSize:12}}>Upload Photo</Btn>{value&&<Btn onClick={()=>onChange("")} v="ghost" style={{fontSize:12,marginLeft:6,color:"#b91c1c"}}>Remove</Btn>}</div><input ref={ref} type="file" accept="image/*" style={{display:"none"}} onChange={hf} /></div>;
}
function Modal({title,onClose,children,wide=false}) {
  return <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:9000,padding:16}}><div style={{background:"white",borderRadius:18,padding:28,width:"95%",maxWidth:wide?820:560,maxHeight:"90vh",overflowY:"auto",fontFamily:F}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}><div style={{fontWeight:"bold",color:"#3b0764",fontSize:16}}>{title}</div><button onClick={onClose} style={{background:"#f1f5f9",border:"none",borderRadius:8,padding:"6px 14px",cursor:"pointer",fontSize:18}}>✕</button></div>{children}</div></div>;
}
...
function LiveClock() {
  ...
  return (
    <div style={{textAlign:"right",fontFamily:F,lineHeight:1.3}}>
      <div style={{fontSize:20,fontWeight:"bold",color:"#3b0764",letterSpacing:2,fontVariantNumeric:"tabular-nums"}}>{timeStr}</div>
...
function Logo({size=60,src}) {
  ...
  return (
    <div style={{width:size,height:size,borderRadius:"50%",overflow:"hidden",border:"3px solid #3b0764",display:"inline-flex",alignItems:"center",justifyContent:"center",background:"white",flexShrink:0}}>
      {logoSrc ? <img src={logoSrc} alt="TNKS Logo" style={{width:"100%",height:"100%",objectFit:"cover"}} onError={()=>setFallback(true)} />
               : <div style={{fontSize:size*.32,color:"#3b0764",fontWeight:"bold",textAlign:"center",fontFamily:F}}>TNKS</div>}
    </div>
  );
}
...
function Sidebar({view,setView,user,onLogout,logo,collapsed=false,onToggleCollapse,myClass}) {
  ...
  const ADMIN_GROUPS = [
    ...
    },
    {
      ...
      items:[
        {id:"transport",          label:"Transport Overview",  icon:"🚌"},
        {id:"student_routes",     label:"Student Routes",      icon:"📍"},
        {id:"vehicles",           label:"Vehicles",            icon:"🚐"},
        {id:"transport_reports",  label:"Transport Reports",   icon:"📊"},
      ]
    },
    {
  ...
  const SECRETARY_GROUPS = [
    ...
    {id:"transport_grp", label:"Transport", icon:"🚌", items:[{id:"transport",label:"Transport Overview",icon:"🚌"},{id:"student_routes",label:"Student Routes",icon:"📍"},{id:"vehicles",label:"Vehicles",icon:"🚐"},{id:"transport_reports",label:"Transport Reports",icon:"📊"}]},
    {id:"school_life_grp", label:"School Life", icon:"🌟",
      items:[
        {id:"events",   label:"Events",   icon:"🎉"},
        {id:"calendar", label:"Calendar", icon:"📅"},
        ...
        <div style={{background:"white",borderRadius:20,boxShadow:"0 24px 64px rgba(0,0,0,.25)",padding:36}}>
          ...
          <div style={{display:"flex",gap:6,marginBottom:20,background:"#f1f5f9",borderRadius:10,padding:4}}>
            {[["staff","👨‍🏫 Staff / Admin"],["parent","👨‍👩‍👧 Parent"]].map(([m,l])=>(
              <button key={m} onClick={()=>{setMode(m);setErr("");}} style={{flex:1,padding:"9px 8px",border:"none",borderRadius:8,cursor:"pointer",fontFamily:F,fontSize:13,fontWeight:mode===m?"bold":"normal",background:mode===m?"white":"transparent",color:mode===m?"#3b0764":"#64748b",boxShadow:mode===m?"0 1px 4px rgba(0,0,0,.1)":"none",transition:"all .2s"}}>{l}</button>
            ))}
          </div>
    ...
    <div style={{padding:24}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20,flexWrap:"wrap",gap:12}}>
        <div style={{display:"flex",alignItems:"center",gap:14}}>
          ...
          <div><h2 style={{margin:0,color:"#3b0764",fontSize:20,fontFamily:F}}>{SCHOOL.name}</h2><div style={{fontSize:12,color:"#64748b"}}>Academic Management Dashboard — CBC System</div></div>
        </div>
...
function StudentsPage({students,setStudents,results,setResults,comments,setComments,fees,setFees,monitoring,setMonitoring,logo,preFilterClass}) {
  const blank={name:"",admNo:"",class:"Grade 7",gender:"Male",photo:"",studentType:"Day Scholar",email:"",parentPassword:"",dob:"",parentName:"",parentPhone:"",address:""};
  ...
  function doEdit(s){setForm({name:s.name,admNo:s.admNo,class:s.class,gender:s.gender||"Male",photo:s.photo||"",studentType:s.studentType||"Day Scholar",email:s.email||"",parentPassword:s.parentPassword||"",dob:s.dob||"",parentName:s.parentName||"",parentPhone:s.parentPhone||"",address:s.address||""});setEditId(s.id);setTab("add");}
  function doDel(id){
  ...
  return (
    <div style={{padding:24}}>
      ...
      {tab==="add"&&<Card style={{marginBottom:20}}><div style={{fontWeight:"bold",color:"#1e293b",marginBottom:14,fontSize:14}}>{editId?"Edit Learner":"Add New Learner"}</div><div style={{marginBottom:14}}><PhotoUp value={form.photo} onChange={v=>setForm({...form,photo:v})}/></div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:12}}><Inp label="FULL NAME *" value={form.name} onChange={v=>setForm({...form,name:v})} placeholder="Full name"/><Inp label="ADM. NUMBER *" value={form.admNo} onChange={v=>setForm({...form,admNo:v})} placeholder="TNKS/001/2025"/><Sel label="CLASS" value={form.class} onChange={v=>setForm({...form,class:v})} options={ALL_CLASSES}/><Sel label="GENDER" value={form.gender} onChange={v=>setForm({...form,gender:v})} options={["Male","Female"]}/><Sel label="STUDENT TYPE" value={form.studentType||"Day Scholar"} onChange={v=>setForm({...form,studentType:v})} options={["Day Scholar","Boarder"]}/><Inp label="DATE OF BIRTH" value={form.dob} onChange={v=>setForm({...form,dob:v})} placeholder="DD/MM/YYYY"/><Inp label="PARENT/GUARDIAN" value={form.parentName} onChange={v=>setForm({...form,parentName:v})} placeholder="Parent full name"/><Inp label="PARENT PHONE" value={form.parentPhone} onChange={v=>setForm({...form,parentPhone:v})} placeholder="+254 7..."/><Inp label="HOME ADDRESS" value={form.address} onChange={v=>setForm({...form,address:v})} placeholder="Village/Location"/></div><div style={{marginTop:12,paddingTop:12,borderTop:"1px dashed #e2e8f0"}}><div style={{fontSize:12,fontWeight:"bold",color:"#3b0764",marginBottom:10}}>👨‍👩‍👧 Parent Portal Access</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}><Inp label="LEARNER EMAIL (parent login)" value={form.email} onChange={v=>setForm({...form,email:v})} placeholder="john@gmail.com" type="email"/><Inp label="PARENT PASSWORD" value={form.parentPassword} onChange={v=>setForm({...form,parentPassword:v})} placeholder="Set password" type="password"/></div></div>{msg.t&&<div style={{marginTop:10,fontSize:13,color:msg.ok?"#1d4ed8":"#b91c1c",fontWeight:"bold"}}>{msg.t}</div>}<div style={{display:"flex",gap:8,marginTop:14}}><Btn onClick={doSave} v="primary">{editId?"Update":"Add Learner"}</Btn>{editId&&<Btn onClick={()=>{setEditId(null);setForm(blank);setTab("list");}} v="ghost">Cancel</Btn>}</div></Card>}
      {tab==="list"&&<><div style={{display:"flex",gap:10,marginBottom:14,flexWrap:"wrap",alignItems:"center"}}><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search by name or adm. no..." style={{flex:1,minWidth:200,border:"1.5px solid #e2e8f0",borderRadius:9,padding:"8px 12px",fontSize:13,fontFamily:F,outline:"none"}}/><Sel value={filterCls} onChange={setFilterCls} options={["All",...ALL_CLASSES]}/><span style={{fontSize:12,color:"#64748b"}}>{filtered.length} learner(s)</span></div><Card style={{padding:0}}><div style={{overflowX:"auto",WebkitOverflowScrolling:"touch"}}><table style={{width:"100%",borderCollapse:"collapse",minWidth:600}}><thead><tr>{["","#","Adm.No","Name","Class","Type","Gender","Parent","Portal","Actions"].map(h=><th key={h} style={th}>{h}</th>)}</tr></thead><tbody>{filtered.length?filtered.map((s,i)=><tr key={s.id} style={{background:i%2===0?"white":"#fafafa"}}><td style={{...td,width:46}}><Avatar name={s.name} photo={s.photo} size={34}/></td><td style={{...td,color:"#94a3b8"}}>{i+1}</td><td style={{...td,fontFamily:"monospace",fontSize:11}}>{s.admNo}</td><td style={{...td,fontWeight:"bold"}}>{s.name}</td><td style={td}><span style={{background:"#f5f3ff",color:"#7c3aed",fontSize:10,padding:"2px 8px",borderRadius:20,fontWeight:"bold"}}>{s.class}</span></td><td style={td}><span style={{fontSize:10,padding:"2px 8px",borderRadius:20,fontWeight:"bold",background:s.studentType==="Boarder"?"#fef3c7":(s.studentType||"Day Scholar")==="Day Scholar"?"#f0fdf4":"#eff6ff",color:s.studentType==="Boarder"?"#b45309":(s.studentType||"Day Scholar")==="Day Scholar"?"#166534":"#1d4ed8"}}>{s.studentType||"Day Scholar"}</span></td><td style={td}>{s.gender||"—"}</td><td style={{...td,fontSize:11}}>{s.parentName||"—"}</td><td style={td}>{s.email?<span style={{fontSize:10,color:"#1d4ed8"}}>✅</span>:<span style={{fontSize:10,color:"#94a3b8"}}>—</span>}</td><td style={td}><button onClick={()=>doEdit(s)} style={{color:"#7c3aed",background:"none",border:"none",cursor:"pointer",fontSize:12,marginRight:8}}>Edit</button><button onClick={()=>doDel(s.id)} style={{color:"#b91c1c",background:"none",border:"none",cursor:"pointer",fontSize:12}}>Del</button></td></tr>):<tr><td colSpan={9} style={{padding:40,textAlign:"center",color:"#94a3b8"}}>No learners found.</td></tr>}</tbody></table></div></Card></>}
...
// RESULTS ENTRY
// ══════════════════════════════════════════════════════════
function ResultsPage({students,results,setResults,comments,setComments,users,term,setTerm,year,setYear,examType,setExamType,user,ttSetup}) {
  const [clsState,setClsState]=useState(""); const [sub,setSub]=useState(""); const [marks,setMarks]=useState({});
  ...
  const [dirty,setDirty]=useState(false); const [refreshing,setRefreshing]=useState(false);
  // ── ROLE-BASED ACCESS: compute first so we can derive sensible defaults ────
  const isAdmin=user?.role==="admin";
  // Pull subject→teacher map straight from timetable — no separate Subject Allocation needed
  const subjectMap=ttSetup?.setupData?.subjectTeachers||PRELOADED_TEACHERS;
  const myAssignments=isAdmin?null:Object.entries(subjectMap).filter(([k,t])=>getTeachersForKey(subjectMap,k).includes(user?.name)).map(([k])=>k);
  const allowedClasses=isAdmin?ALL_CLASSES:ALL_CLASSES.filter(c=>myAssignments.some(k=>k.startsWith(c+"::")));
  // FIX: auto-select teacher's first assigned class instead of always hardcoding "Grade 7".
  // Admins keep Grade 7 as the default; teachers land on their own class immediately.
  const cls=clsState||(isAdmin?"Grade 7":(allowedClasses[0]||"Grade 7"));
  const subs=getSubs(cls);
  // For combined results subjects (e.g. "Social Studies (History & Geography)"), check if the teacher
  // is assigned to ANY of the component timetable parts (e.g. History OR Geography)
  function teacherCanDoResultsSub(cls,sub){
    if(isAdmin) return true;
    if((myAssignments||[]).includes(`${cls}::${sub}`)) return true;
    const parts=COMBINED_SUBJECT_PARTS[sub]||[];
    return parts.some(p=>(myAssignments||[]).includes(`${cls}::${p}`));
  }
  const allowedSubs=isAdmin?subs:subs.filter(s=>teacherCanDoResultsSub(cls,s));
  // FIX: default to the first subject the teacher is actually assigned to, not subs[0] ("English").
  const curSub=sub||(allowedSubs.length?allowedSubs[0]:subs[0])||"";
  const canEdit=isAdmin||teacherCanDoResultsSub(cls,curSub);
  ...
  const _resultKey=results.filter(r=>r.class===cls&&r.subject===curSub&&r.term===term&&r.year===year&&r.examType===examType).map(r=>`${r.studentId}:${r.marks}`).sort().join("|");
  useEffect(()=>{const m={}; clsStu.forEach(s=>{const r=results.find(r=>r.studentId===s.id&&r.class===cls&&r.subject===curSub&&r.term===term&&r.year===year&&r.examType===examType); if(r) m[s.id]=r.marks;}); setMarks(m);},[cls,curSub,term,year,examType,_resultKey]);
  ...
  async function doRefresh(){
    setRefreshing(true);
    const fresh=await load("tnks_results")||[];
    setResults(fresh);
    setDirty(false);
  ...
  async function doSave(){
    setMsg("⏳ Saving...");
    // FIX: fetch the latest results from Supabase before saving so we don't
    // overwrite marks entered by other teachers in parallel sessions.
    const fresh=await load("tnks_results")||[];
    // Merge: discard only the slice belonging to this class/subject/term/year/examType
    const next=[...fresh.filter(r=>!(r.class===cls&&r.subject===curSub&&r.term===term&&r.year===year&&r.examType===examType))];
    clsStu.forEach(s=>{
      const m=marks[s.id];
  ...
  const th={textAlign:"left",padding:"10px 14px",fontWeight:"bold",fontSize:11,color:"#7c3aed",background:"#f5f3ff",letterSpacing:.5};
  ...
  return (
    <div style={{padding:24}}>
      ...
      {dirty&&<div style={{background:"#fef3c7",border:"1px solid #f59e0b",borderRadius:10,padding:"10px 16px",marginBottom:12,display:"flex",alignItems:"center",gap:10}}><span style={{fontSize:16}}>⚠️</span><span style={{fontSize:13,color:"#92400e",fontWeight:"bold",flex:1}}>You have unsaved marks — click 💾 Save Results before leaving this page.</span></div>}
      <Card style={{marginBottom:18}}><div style={{display:"flex",gap:12,flexWrap:"wrap",alignItems:"flex-end"}}>
        <Sel label="CLASS" value={cls} onChange={v=>{if(dirty&&!window.confirm("You have unsaved marks. Discard and continue?")) return; setDirty(false); setClsState(v); setSub("");}} options={allowedClasses}/>
        <Sel label="SUBJECT" value={curSub} onChange={v=>{if(dirty&&!window.confirm("You have unsaved marks. Discard and continue?")) return; setDirty(false); setSub(v);}} options={allowedSubs.length?allowedSubs:subs}/>
        {/* Admin/Dean can change term/year/examType; teachers see them as read-only — set via Exam Settings */}
        {isAdmin ? <>
          <Sel label="TERM" value={term} onChange={v=>{if(dirty&&!window.confirm("You have unsaved marks. Discard and continue?")) return; setDirty(false); setTerm(v);}} options={TERMS}/>
          <Sel label="EXAM TYPE" value={examType} onChange={v=>{if(dirty&&!window.confirm("You have unsaved marks. Discard and continue?")) return; setDirty(false); setExamType(v);}} options={EXAM_TYPES}/>
          <Sel label="YEAR" value={year} onChange={v=>{if(dirty&&!window.confirm("You have unsaved marks. Discard and continue?")) return; setDirty(false); setYear(v);}} options={YEARS}/>
        </> : <div style={{display:"flex",alignItems:"center",gap:8,background:"#f5f3ff",borderRadius:8,padding:"8px 14px",fontSize:12,color:"#7c3aed",fontWeight:"bold",border:"1.5px solid #e9d5ff"}}>
          📌 {term} &nbsp;·&nbsp; {examType} &nbsp;·&nbsp; {year}
          <span style={{fontSize:10,color:"#a78bfa",fontWeight:"normal",marginLeft:4}}>Set by Admin/Dean</span>
        </div>}
        <Btn onClick={doRefresh} v="ghost" style={{fontSize:12,padding:"8px 14px",alignSelf:"flex-end"}}>{refreshing?"⏳ Refreshing...":"🔄 Refresh"}</Btn>
      </div></Card>
      {tab==="entry"&&<Card style={{padding:0}}>
        {/* Active exam banner */}
        <div style={{padding:"8px 16px",background:"#ede9fe",borderBottom:"1px solid #c4b5fd",display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
          <span style={{fontSize:11,fontWeight:"bold",color:"#7c3aed"}}>📌 ACTIVE EXAM PERIOD:</span>
          <span style={{fontSize:12,color:"#3b0764",fontWeight:"bold"}}>{term} &nbsp;·&nbsp; {examType} &nbsp;·&nbsp; {year}</span>
          <span style={{fontSize:10,color:"#7c3aed",marginLeft:4}}>(Set by Admin/Dean in Exam Settings)</span>
        </div>
        {/* ── Live stats bar — updates as marks are typed ── */}
        {(()=>{
          const entered=clsStu.filter(s=>marks[s.id]!==""&&marks[s.id]!==undefined&&!isNaN(parseFloat(marks[s.id])));
          const vals=entered.map(s=>parseFloat(marks[s.id]));
          const total=vals.reduce((a,b)=>a+b,0);
          const mean=vals.length?total/vals.length:null;
          const highest=vals.length?Math.max(...vals):null;
          const lowest=vals.length?Math.min(...vals):null;
          const passing=vals.filter(v=>v>=41).length;
          const statBox=(icon,label,value,col="#3b0764")=>(
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",padding:"8px 14px",borderRight:"1px solid #e9d5ff",flex:1,minWidth:70}}>
              <div style={{fontSize:10,color:"#7c3aed",fontWeight:"bold",marginBottom:2,letterSpacing:.4}}>{icon} {label}</div>
              <div style={{fontSize:17,fontWeight:"bold",color:col,fontFamily:F}}>{value}</div>
            </div>
          );
          return clsStu.length>0&&(
            <div style={{display:"flex",background:"#f5f3ff",borderBottom:"1px solid #e9d5ff",flexWrap:"wrap"}}>
              {statBox("✏️","ENTERED",`${entered.length} / ${clsStu.length}`,"#3b0764")}
              {statBox("∑","TOTAL",vals.length?total:"—","#1d4ed8")}
              {statBox("μ","MEAN",mean!==null?mean.toFixed(1)+"%":"—",mean===null?"#94a3b8":mean>=75?"#1d4ed8":mean>=41?"#7c3aed":"#b91c1c")}
              {statBox("▲","HIGHEST",highest!==null?highest:"—","#166534")}
              {statBox("▼","LOWEST",lowest!==null?lowest:"—","#b91c1c")}
              {statBox("✅","PASSING",vals.length?`${passing} (${Math.round(passing/vals.length*100)}%)`:"—","#0e7490")}
            </div>
          );
        })()}
        {/* Marks table — name column is sticky so it never scrolls out of view */}
        <div style={{overflowX:"auto",WebkitOverflowScrolling:"touch"}}>
          <table style={{width:"100%",borderCollapse:"collapse",tableLayout:"fixed"}}>
            <colgroup>
              <col style={{width:32}}/>
              <col style={{width:"40%",minWidth:140}}/>
              <col style={{width:"25%",minWidth:100}}/>
              <col style={{width:"12%",minWidth:60}}/>
              <col style={{width:"8%",minWidth:40}}/>
              <col style={{width:"15%",minWidth:60}}/>
            </colgroup>
            <thead>
              <tr>
                <th style={{...th,position:"sticky",left:0,zIndex:2,background:"#f5f3ff",padding:"10px 6px",textAlign:"center"}}>#</th>
                <th style={{...th,position:"sticky",left:32,zIndex:2,background:"#f5f3ff",padding:"10px 10px",whiteSpace:"nowrap"}}>LEARNER NAME</th>
                <th style={th}>MARKS (0–100)</th>
            ...
            <tbody>
              {clsStu.length?clsStu.map((s,i)=>{
                const m=marks[s.id];
                const g=m!==""&&m!==undefined&&!isNaN(parseFloat(m))?getGrade(parseFloat(m)):null;
                return(
                  <tr key={s.id} style={{background:i%2===0?"white":"#fafafa"}}>
                    <td style={{...td,color:"#94a3b8",textAlign:"center",padding:"8px 6px",position:"sticky",left:0,background:i%2===0?"white":"#fafafa",zIndex:1}}>{i+1}</td>
                    <td style={{...td,fontWeight:"bold",fontSize:12,padding:"8px 10px",position:"sticky",left:32,background:i%2===0?"white":"#fafafa",zIndex:1,whiteSpace:"normal",wordBreak:"break-word",lineHeight:1.3}}>
                      <div style={{fontSize:12,fontWeight:"bold",color:"#1e293b"}}>{s.name}</div>
                      <div style={{fontSize:10,color:"#94a3b8",marginTop:1}}>{s.admNo||""}</div>
                    </td>
                    <td style={{...td,textAlign:"center"}}>
                      <input type="number" min={0} max={100} value={m||""}
                        onChange={e=>{const n=parseFloat(e.target.value); setMarks(p=>({...p,[s.id]:isNaN(n)?""  :Math.min(100,Math.max(0,n))})); setDirty(true);}}
                        style={{width:"100%",maxWidth:90,border:"1.5px solid #e2e8f0",borderRadius:6,padding:"7px 6px",fontSize:14,fontFamily:F,textAlign:"center",outline:"none",fontWeight:"bold",color:"#3b0764",background:canEdit?"white":"#f8f8f8",cursor:canEdit?"text":"not-allowed",boxSizing:"border-box"}}
                        placeholder="—" disabled={!canEdit}/>
                    </td>
                    <td style={{...td,textAlign:"center"}}>{g?<span style={{background:g.bg,color:g.col,fontSize:11,padding:"2px 7px",borderRadius:20,fontWeight:"bold",whiteSpace:"nowrap"}}>{g.g}</span>:"—"}</td>
                    <td style={{...td,textAlign:"center",fontWeight:"bold",color:"#7c3aed"}}>{g?g.pts:"—"}</td>
                    <td style={{...td,textAlign:"center"}}>
                      <button onClick={()=>{setCmStu(s);const ex=(comments||[]).find(c=>c.studentId===s.id&&c.term===term&&c.year===year&&c.examType===examType);setCmText(ex?ex.text:"");setCmMode(true);}}
                        style={{background:"#f5f3ff",border:"none",borderRadius:6,padding:"5px 10px",cursor:"pointer",fontSize:11,color:"#7c3aed",fontFamily:F}}>💬</button>
                    </td>
                  </tr>
                );
              }):<tr><td colSpan={6} style={{padding:30,textAlign:"center",color:"#94a3b8"}}>No learners in {cls}.</td></tr>}
            </tbody>
            {/* ── tfoot: live totals row ── */}
            {(()=>{
              const vals=clsStu.map(s=>marks[s.id]).filter(v=>v!==""&&v!==undefined&&!isNaN(parseFloat(v))).map(parseFloat);
              if(!vals.length) return null;
              const total=vals.reduce((a,b)=>a+b,0);
              const mean=total/vals.length;
              const mg=getGrade(mean);
              return(
                <tfoot>
                  <tr style={{background:"#ede9fe",borderTop:"2px solid #c4b5fd"}}>
                    <td colSpan={2} style={{padding:"9px 12px",fontWeight:"bold",fontSize:12,color:"#3b0764",position:"sticky",left:0,background:"#ede9fe",zIndex:1}}>
                      CLASS TOTALS ({vals.length} entered)
                    </td>
                    <td style={{padding:"9px 12px",textAlign:"center"}}>
                      <div style={{fontSize:11,color:"#7c3aed",fontWeight:"bold"}}>∑ {total} &nbsp;|&nbsp; μ {mean.toFixed(1)}%</div>
                    </td>
                    <td style={{padding:"9px 12px",textAlign:"center"}}>
                      <span style={{background:mg.bg,color:mg.col,fontSize:11,padding:"2px 7px",borderRadius:20,fontWeight:"bold"}}>{mg.g}</span>
                    </td>
                    <td style={{padding:"9px 12px",textAlign:"center",fontWeight:"bold",color:"#7c3aed"}}>{mg.pts}</td>
                    <td/>
                  </tr>
                </tfoot>
              );
        ...
        {clsStu.length>0&&<div style={{padding:"12px 16px",borderTop:"1px solid #f1f5f9",display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
          {canEdit?<Btn onClick={doSave} v="blue">💾 Save Results</Btn>:<span style={{fontSize:12,color:"#b91c1c",fontWeight:"bold"}}>🔒 You are not assigned to {curSub} — {cls}</span>}
          {msg&&<span style={{color:"#1d4ed8",fontWeight:"bold",fontSize:13}}>{msg}</span>}
        </div>}
      </Card>}
...
function buildSectionHeader(logo) {
  ...
  return `<div style="text-align:center;border-bottom:3px double #3b0764;padding-bottom:10px;margin-bottom:16px;">
    ${logoTag}
...
function buildStampBox() {
  return `<div class="stamp-wrap" style="display:flex;justify-content:flex-end;margin-top:10px;">
    <div style="border:2px dashed #3b0764;border-radius:8px;width:160px;height:70px;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:6px;">
      <div style="font-size:9px;font-weight:bold;color:#3b0764;letter-spacing:0.5px;">OFFICIAL STAMP</div>
      <div style="font-size:8px;color:#94a3b8;margin-top:14px;">Date: _______________</div>
    </div>
  </div>`;
}
function buildHTMLDoc(title, bodyHTML, logo) {
  const logoWm = logo
    ...
    : `<div style="position:fixed;top:50%;left:50%;transform:translate(-50%,-50%) rotate(-35deg);opacity:0.04;pointer-events:none;z-index:0;font-size:80px;font-weight:900;color:#3b0764;white-space:nowrap;font-family:Georgia,serif;">${SCHOOL.motto}</div>`;
    ...
    *{box-sizing:border-box;}
    ...
    @media print{
      *{-webkit-print-color-adjust:exact !important;print-color-adjust:exact !important;color-adjust:exact !important;}
      ...
      /* Each class block: one page only, no breaks inside */
      .print-page-block{
      ...
      /* Stamp must never break away from the table */
      .stamp-wrap{
  ...
  <script>
    window.addEventListener('load', function() {
      // A4 portrait printable area with 8mm margins: 194mm wide x 281mm tall
      var PAGE_W_PX = (210 - 16) * (96 / 25.4); // ~734px
      var PAGE_H_PX = (297 - 16) * (96 / 25.4); // ~1073px
      ...
      function scaleToFit() {
        ...
        blocks.forEach(function(block) {
          ...
          var W = block.scrollWidth;
          var H = block.scrollHeight;

          var scaleW = W > PAGE_W_PX ? PAGE_W_PX / W : 1;
          var scaleH = H > PAGE_H_PX ? PAGE_H_PX / H : 1;
          var scale = Math.min(scaleW, scaleH);

          if (scale < 1) {
            block.style.transformOrigin = 'top left';
            block.style.transform = 'scale(' + scale + ')';
            // Collapse the extra space the scaled-down block would leave
            block.style.width = (100 / scale) + '%';
            block.style.marginBottom = '-' + (H * (1 - scale)) + 'px';
          }
        });
      }
...
function printViaMedianPDF(title, html) {
  ...
  toolbar.style.cssText = "display:flex;align-items:center;justify-content:space-between;padding:10px 14px;background:#3b0764;flex-shrink:0;gap:10px;";
...
function ReportsPage({students,results,comments,term,setTerm,year,setYear,examType,setExamType,logo,users,ttSetup}) {
  ...
  function getClassRankings(classStudents) {
    ...
    const displaySubs=getPrintSubs(className);
    return classStudents.map(s=>{
      ...
      // Use display totals (SCI+AGRI averaged as one column) — same as what prints
      const dispMarks=displaySubs.map(su=>getPrintMark(sr,su)).filter(v=>v!==null);
      const total=dispMarks.reduce((a,b)=>a+b,0);
      const avg=dispMarks.length?total/dispMarks.length:0;
      return {...s,total,avg,subs:sr};
    }).sort((a,b)=>b.total-a.total).map((s,_,arr)=>{
      // Competition ranking: ties get same position, next gets skipped
      const pos=arr.filter(x=>x.total>s.total).length+1;
  ...
  function getSubjectTeacherName(className, subject) {
    ...
    if(tid) {
      // Could be a user ID, name string, or array of names — resolve against live users list
      const names = Array.isArray(tid) ? tid : [tid];
      const knownUsers=(users||[]).filter(u=>u.staffType==="teaching"||u.role==="teacher");
      const resolved=names.map(n=>{
        if(!n) return null;
        // 1. Match by user ID (stable reference)
        const byId=knownUsers.find(u=>u.id===n);
        if(byId) return byId.name;
        // 2. Match by exact name
        const byName=knownUsers.find(u=>u.name===n);
        if(byName) return byName.name;
        // 3. Case-insensitive name match
        const byNameCI=knownUsers.find(u=>(u.name||"").toLowerCase()===(n||"").toLowerCase());
        if(byNameCI) return byNameCI.name;
        // 4. Ghost name: stored name no longer exists in users — suppress it silently
        //    (happens when a teacher was renamed/deleted; saveEdit now keeps map in sync going forward)
        if(knownUsers.length>0) return null;
        return n; // no users defined yet — keep raw string as fallback
      }).filter(Boolean);
      // Deduplicate (same teacher stored multiple times under old/new names)
      return [...new Set(resolved)].join(", ");
    }
    // Try classes.subjects
    const cls=(sd.classes||[]).find(c=>c.name===className);
    if(cls) { const sub=(cls.subjects||[]).find(s=>s.subject===subject); if(sub?.teacherName) {
      const u=(users||[]).find(u=>u.name===sub.teacherName||u.id===sub.teacherName);
      return u?u.name:sub.teacherName;
    }}
    return "";
  }
  function getClassTeacherName(className) {
    ...
    if(tid){
      const byId=(users||[]).find(u=>u.id===tid);
      if(byId) return byId.name;
      const byName=(users||[]).find(u=>u.name===tid||(u.name||"").toLowerCase()===(tid||"").toLowerCase());
      if(byName) return byName.name;
      return tid;
    }
    const cls=(sd.classes||[]).find(c=>c.name===className);
    if(cls?.classTeacherName){
      const u=(users||[]).find(u=>u.name===cls.classTeacherName||u.id===cls.classTeacherName);
      return u?u.name:cls.classTeacherName;
    }
    return "";
  }
  ...
  // ── Grade 4-6 Print Helpers: Integrated Science & Agriculture combined ──────
  const SCI_AGRI_COMBINED = "Integrated Science & Agriculture";
  const SCI_AGRI_COL      = "#0d9488"; // teal header
  const SCI_AGRI_BG       = "#f0fdfa"; // teal-50 row highlight
  const SCI_AGRI_HDR      = "#0d9488"; // teal column header
  const SCI_AGRI_DARK     = "#0f766e"; // darker teal for text

  function isGrade46(cls) {
    const n = parseInt((cls||"").split(" ")[1]);
    return n >= 4 && n <= 6;
  }
  // Returns 6 display subjects for Grade 4-6 (Int Sci + Agri → combined), normal for others
  function getPrintSubs(className) {
    const subs = getSubs(className);
    if(!isGrade46(className)) return subs;
    const out = []; let done = false;
    for(const s of subs) {
      if(s === "Integrated Science" || s === "Agriculture and Nutrition") {
        if(!done) { out.push(SCI_AGRI_COMBINED); done = true; }
      } else out.push(s);
    }
    return out;
  }
  // Get mark for a display subject (averages Int Sci & Agri for combined)
  function getPrintMark(sr, displaySub) {
    if(displaySub === SCI_AGRI_COMBINED) {
      const r1 = sr.find(x => x.subject === "Integrated Science");
      const r2 = sr.find(x => x.subject === "Agriculture and Nutrition");
      const vals = [r1, r2].filter(Boolean).map(x => x.marks);
      return vals.length ? vals.reduce((a,b)=>a+b,0)/vals.length : null;
    }
    const r = sr.find(x => x.subject === displaySub);
    return r != null ? r.marks : null;
  }
  // Get marks array for all display subjects (for total/mean)
  function getPrintMarks(sr, displaySubs) {
    return displaySubs.map(s => getPrintMark(sr, s)).filter(v => v !== null);
  }
  // Get all marks for a display subject across all ranked students (for summary rows)
  function getColMarks(ranked, displaySub) {
    return ranked.map(s => getPrintMark(s.subs, displaySub)).filter(v => v !== null);
  }
  // ── End Grade 4-6 helpers ─────────────────────────────────────────────────

  function buildReportHTML(student, allInClass) {
    const subs=getPrintSubs(student.class);
    ...
    // For Grade 4-6: use 6 display subjects for total/avg; others use raw results
    const dispMarks=getPrintMarks(sr,subs);
    const total=dispMarks.reduce((a,b)=>a+b,0);
    const avg=dispMarks.length?total/dispMarks.length:0;
    ...
    const rows=subs.map((s,idx)=>{
      const rawMark=getPrintMark(sr,s);
      const isComb=s===SCI_AGRI_COMBINED;
      const g=rawMark!==null?getGrade(rawMark):null;
      const displayMark=rawMark!==null?(isComb?rawMark.toFixed(1):rawMark):"—";
      const remark=rawMark!==null?getAutoRemark(isComb?"Integrated Science":s,rawMark):"—";
      const subTeachers=isComb
        ?[...new Set(["Integrated Science","Agriculture and Nutrition"].flatMap(p=>getSubjectTeachersForResult(student.class,p)).filter(Boolean))]
        :getSubjectTeachersForResult(student.class,s);
      ...
      const rowBg=isComb?SCI_AGRI_BG:(idx%2===0?"rgba(255,255,255,0.5)":"rgba(248,250,252,0.5)");
      const labelStyle=isComb?`color:${SCI_AGRI_DARK};font-weight:bold;`:"";
      return `<tr style="background:${rowBg}">
        <td style="padding:6px 8px;font-size:10px;${labelStyle}">${s}</td>
        <td style="padding:6px 8px;font-weight:bold;text-align:center;">${displayMark}</td>
    ...
    return `<div style="page-break-after:always;page-break-inside:avoid;padding:8px 14px;max-width:700px;margin:0 auto;position:relative;font-size:9.5px;">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;font-size:9.5px;margin-bottom:7px;border:1px solid #e2e8f0;border-radius:6px;padding:5px 8px;background:rgba(255,255,255,0.6);">
      ...
      <table style="width:100%;border-collapse:collapse;font-size:9px;margin-bottom:7px;">
        ...
        <tbody>${rows}
          <tr style="background:rgba(245,243,255,0.6);font-weight:bold;border-top:2px solid #3b0764;">
            <td style="padding:4px 6px;">TOTAL</td>
            <td style="padding:4px 6px;text-align:center;font-size:11px;">${dispMarks.length?total.toFixed(0):"—"}</td>
            <td colspan="4" style="padding:4px 6px;font-size:8px;color:#64748b;">Sum of all subject marks</td>
          </tr>
          <tr style="background:rgba(245,243,255,0.7);font-weight:bold;">
      ...
      <div style="border:1px solid #e2e8f0;border-radius:6px;padding:5px 8px;margin-bottom:7px;font-size:9px;background:rgba(255,255,255,0.6);">
      ...
      <div style="margin-top:7px;background:rgba(248,250,252,0.6);border-radius:6px;padding:4px 8px;display:flex;flex-wrap:wrap;align-items:center;gap:3px;">
  ...
  function buildClassResultsHTML(classStudents, className) {
    const subs=getPrintSubs(className);
    const ranked=getClassRankings(classStudents);
    const rows=ranked.map((s,i)=>{
      const sr=s.subs;
      const dispM=subs.map(su=>getPrintMark(sr,su));
      const dispTotal=dispM.filter(v=>v!==null).reduce((a,b)=>a+b,0);
    ...
    return `<div class="print-page-block" style="margin-bottom:32px;">
      ${buildSectionHeader(logo)}
      ...
      <table class="results-table" style="width:100%;border-collapse:collapse;font-size:14px;min-width:600px;table-layout:auto;">
        <thead><tr style="background:#3b0764;">
          ...
          ${subs.map(s=>{const isComb=s===SCI_AGRI_COMBINED;const short=getSubShort(s);return`<th style="padding:4px 3px;text-align:center;max-width:70px;white-space:nowrap;vertical-align:bottom;background:${isComb?"#0d9488":"#3b0764"};"><div style="writing-mode:vertical-rl;text-orientation:mixed;transform:rotate(180deg);font-size:20px;font-weight:900;line-height:2.3;height:80px;display:flex;align-items:flex-end;justify-content:center;color:white;" title="${s}">${short}</div></th>`;}).join("")}
        ...
        <tbody>
          ...
          <tr><td colspan="${subs.length+3}" style="padding:3px;border:none;background:rgba(255,255,255,0);"></td></tr>
          <tr><td colspan="${subs.length+3}" style="padding:3px;border:none;background:rgba(255,255,255,0);"></td></tr>
          <tr class="summary-row" style="background:rgba(254,243,199,0.7);font-weight:bold;border-top:2px solid #3b0764;">
            <td style="padding:5px 6px;font-weight:bold;">TOTAL SCORES</td>
            <td style="padding:5px 6px;"></td>
            ${subs.map(su=>{const isComb=su===SCI_AGRI_COMBINED;const allMarks=getColMarks(ranked,su);const tot=allMarks.reduce((a,b)=>a+b,0);return`<td style="padding:5px 4px;text-align:center;font-weight:bold;${isComb?`background:${SCI_AGRI_BG};color:${SCI_AGRI_DARK};`:""}">${allMarks.length?(isComb?tot.toFixed(1):tot.toFixed(0)):"—"}</td>`;}).join("")}
            <td style="padding:5px 6px;text-align:center;font-weight:bold;background:#fef3c7;">${ranked.length?ranked.map(s=>{const dispM=subs.map(su=>getPrintMark(s.subs,su)).filter(v=>v!==null);return dispM.reduce((a,b)=>a+b,0);}).reduce((a,b)=>a+b,0).toFixed(0):"—"}</td>
            <td style="padding:5px 6px;"></td>
          </tr>
          <tr class="mean-row" style="background:rgba(245,243,255,0.7);font-weight:bold;">
            <td style="padding:5px 6px;">MEAN SCORE</td>
            <td style="padding:5px 6px;"></td>
            ${subs.map(su=>{const isComb=su===SCI_AGRI_COMBINED;const allMarks=getColMarks(ranked,su);const m=allMarks.length?allMarks.reduce((a,b)=>a+b,0)/allMarks.length:0;return`<td style="padding:5px 4px;text-align:center;font-weight:bold;${isComb?`background:${SCI_AGRI_BG};color:${SCI_AGRI_DARK};`:""}">${m>0?m.toFixed(2):"—"}</td>`;}).join("")}
            <td style="padding:5px 6px;text-align:center;font-weight:bold;">${ranked.length?(ranked.map(s=>{const dm=subs.map(su=>getPrintMark(s.subs,su)).filter(v=>v!==null);return dm.reduce((a,b)=>a+b,0);}).reduce((a,b)=>a+b,0)/ranked.length).toFixed(2):"—"}</td>
            <td style="padding:5px 6px;"></td>
          </tr>
          <tr class="rank-row" style="background:rgba(240,249,255,0.7);font-weight:bold;">
            <td style="padding:5px 6px;">RANK</td>
            <td style="padding:5px 6px;"></td>
            ${subs.map(su=>{const isComb=su===SCI_AGRI_COMBINED;const allMarks=getColMarks(ranked,su);const m=allMarks.length?allMarks.reduce((a,b)=>a+b,0)/allMarks.length:0;const subMeans=subs.map(s2=>{const am=getColMarks(ranked,s2);return am.length?am.reduce((a,b)=>a+b,0)/am.length:0;});const rank=subMeans.filter(v=>v>m).length+1;return`<td style="padding:5px 4px;text-align:center;font-weight:bold;${isComb?`background:${SCI_AGRI_BG};color:${SCI_AGRI_DARK}`:""}">${m>0?rank:"—"}</td>`;}).join("")}
            <td style="padding:5px 6px;"></td>
            <td style="padding:5px 6px;"></td>
          </tr>
          ${teacherRow}
  ...
  function buildClassGradesHTML(classStudents, className) {
    ...
    const rows=ranked.map((s,i)=>{
      const sr=s.subs;
      const dispM=subs.map(su=>getPrintMark(sr,su)).filter(v=>v!==null);
      const dispAvg=dispM.length?dispM.reduce((a,b)=>a+b,0)/dispM.length:0;
      const og=dispAvg>0?getGrade(dispAvg):null;
      const _np2=(s.name||"").trim().split(/\s+/);const _fn2=_np2[0]||"";const _ln2=_np2.slice(1).join(" ");
      return `<tr style="background:${i%2===0?"rgba(255,255,255,0.55)":"rgba(248,250,252,0.55)"}">
        <td style="padding:5px 6px;font-weight:bold;color:${i<3?"#b45309":"#374151"};text-align:center;">${s.position||"—"}</td>
        <td style="padding:5px 8px;font-weight:bold;font-size:13px;line-height:2.25;white-space:nowrap;min-width:100px;">${_fn2}${_ln2?`<br><span style="font-size:13px;font-weight:bold;color:#374151;">${_ln2}</span>`:""}</td>
        ${subs.map(su=>{const isComb=su===SCI_AGRI_COMBINED;const v=getPrintMark(sr,su);const g=v!==null?getGrade(v):null;return`<td style="padding:5px 4px;text-align:center;font-weight:bold;min-width:32px;width:32px;${isComb?`background:${SCI_AGRI_BG};`:""}">${g?`<span style="background:${g.bg};color:${g.col};font-size:8px;padding:1px 4px;border-radius:6px;font-weight:bold;">${g.g}</span>`:"—"}</td>`;}).join("")}
    ...
    return `<div class="print-page-block" style="margin-bottom:32px;">
      ...
      <table class="grade-table" style="width:100%;border-collapse:collapse;font-size:14px;min-width:600px;table-layout:auto;">
        <thead><tr style="background:#1d4ed8;">
          <th style="padding:5px 6px;text-align:center;color:white;">Pos</th>
          <th style="padding:5px 6px;text-align:left;color:white;">Name</th>
          ${subs.map(s=>{const isComb=s===SCI_AGRI_COMBINED;const short=getSubShort(s);return`<th style="padding:4px 3px;text-align:center;max-width:70px;white-space:nowrap;vertical-align:bottom;background:${isComb?"#0d9488":"#1d4ed8"};"><div style="writing-mode:vertical-rl;text-orientation:mixed;transform:rotate(180deg);font-size:20px;font-weight:900;line-height:2.3;height:80px;display:flex;align-items:flex-end;justify-content:center;color:white;" title="${s}">${short}</div></th>`;}).join("")}
        ...
        <tbody>
          ...
          <tr class="summary-row" style="background:rgba(254,243,199,0.7);font-weight:bold;border-top:2px solid #3b0764;">
            <td style="padding:5px 6px;font-weight:bold;">TOTAL SCORES</td>
            <td style="padding:5px 6px;"></td>
            ${subs.map(su=>{const isComb=su===SCI_AGRI_COMBINED;const allMarks=getColMarks(ranked,su);const tot=allMarks.reduce((a,b)=>a+b,0);const tg=tot>0?getGrade(tot/allMarks.length):null;return`<td style="padding:5px 4px;text-align:center;${isComb?`background:${SCI_AGRI_BG};`:""}">${tg?`<span style="background:${tg.bg};color:${tg.col};font-size:8px;padding:1px 4px;border-radius:6px;font-weight:bold;" title="Total: ${tot.toFixed(1)}">${tg.g}</span>`:"—"}</td>`;}).join("")}
            ${(()=>{const allAvgs=ranked.map(s=>{const dm=subs.map(su=>getPrintMark(s.subs,su)).filter(v=>v!==null);return dm.length?dm.reduce((a,b)=>a+b,0)/dm.length:0;}).filter(v=>v>0);const classAvg=allAvgs.length?allAvgs.reduce((a,b)=>a+b,0)/allAvgs.length:0;const og=classAvg>0?getGrade(classAvg):null;return`<td style="padding:5px 6px;text-align:center;">${og?`<span style="background:${og.bg};color:${og.col};font-size:8px;padding:1px 4px;border-radius:6px;font-weight:bold;">${og.g}</span>`:"—"}</td>`;})()}
          </tr>
          <tr class="mean-row" style="background:rgba(245,243,255,0.7);font-weight:bold;">
            <td style="padding:5px 6px;">MEAN SCORE</td>
            <td style="padding:5px 6px;"></td>
            ${subs.map(su=>{const isComb=su===SCI_AGRI_COMBINED;const allMarks=getColMarks(ranked,su);const m=allMarks.length?allMarks.reduce((a,b)=>a+b,0)/allMarks.length:0;const mg=m>0?getGrade(m):null;return`<td style="padding:5px 4px;text-align:center;${isComb?`background:${SCI_AGRI_BG};`:""}">${ mg?`<span style="background:${mg.bg};color:${mg.col};font-size:8px;padding:1px 4px;border-radius:6px;font-weight:bold;">${mg.g}</span>`:"—"}</td>`;}).join("")}
            ${(()=>{const allAvgs2=ranked.map(s=>{const dm=subs.map(su=>getPrintMark(s.subs,su)).filter(v=>v!==null);return dm.length?dm.reduce((a,b)=>a+b,0)/dm.length:0;}).filter(v=>v>0);const classAvg2=allAvgs2.length?allAvgs2.reduce((a,b)=>a+b,0)/allAvgs2.length:0;const og2=classAvg2>0?getGrade(classAvg2):null;return`<td style="padding:5px 6px;text-align:center;">${og2?`<span style="background:${og2.bg};color:${og2.col};font-size:8px;padding:1px 4px;border-radius:6px;font-weight:bold;">${og2.g}</span>`:"—"}</td>`;})()}
          </tr>
          <tr class="rank-row" style="background:rgba(240,249,255,0.7);font-weight:bold;">
            <td style="padding:5px 6px;">RANK</td>
            <td style="padding:5px 6px;"></td>
    ...
    <div style={{padding:24}}>
      <PageH title="Report Forms" sub="Generate CBC learner progress report cards">
        <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
          ...
          <div style={{position:"relative"}}>
            <Btn onClick={()=>{setShowXl(s=>!s);setShowDl(false);}} v="green" style={{fontSize:12}}>📥 Download Excel ▾</Btn>
            {showXl&&<div style={{position:"fixed",top:"auto",left:"50%",transform:"translateX(-50%)",zIndex:1001,background:"white",border:"1px solid #e2e8f0",borderRadius:12,boxShadow:"0 8px 32px rgba(0,0,0,.18)",padding:8,minWidth:270,marginTop:4,maxHeight:"80vh",overflowY:"auto"}}>
              <div style={{fontSize:10,color:"#94a3b8",padding:"4px 10px",fontWeight:"bold",letterSpacing:.5}}>RESULTS (MARKS) — EXCEL</div>
              <button onClick={()=>{downloadClassExcel(cls,"results");setShowXl(false);}} style={{width:"100%",display:"block",padding:"8px 14px",background:"none",border:"none",cursor:"pointer",textAlign:"left",fontSize:12,fontFamily:F,color:"#374151",borderRadius:8}} onMouseEnter={e=>e.target.style.background="#f0fdf4"} onMouseLeave={e=>e.target.style.background="none"}>📊 {cls} — Results (Excel)</button>
              <button onClick={()=>{downloadSchoolExcel("results");setShowXl(false);}} style={{width:"100%",display:"block",padding:"8px 14px",background:"none",border:"none",cursor:"pointer",textAlign:"left",fontSize:12,fontFamily:F,color:"#374151",borderRadius:8}} onMouseEnter={e=>e.target.style.background="#f0fdf4"} onMouseLeave={e=>e.target.style.background="none"}>📊 All Classes — Results (Excel)</button>
              <div style={{borderTop:"1px solid #f1f5f9",margin:"6px 0"}}/>
              <div style={{fontSize:10,color:"#94a3b8",padding:"4px 10px",fontWeight:"bold",letterSpacing:.5}}>GRADES (CBC BANDS) — EXCEL</div>
              <button onClick={()=>{downloadClassExcel(cls,"grades");setShowXl(false);}} style={{width:"100%",display:"block",padding:"8px 14px",background:"none",border:"none",cursor:"pointer",textAlign:"left",fontSize:12,fontFamily:F,color:"#374151",borderRadius:8}} onMouseEnter={e=>e.target.style.background="#f0fdf4"} onMouseLeave={e=>e.target.style.background="none"}>🎯 {cls} — Grades (Excel)</button>
              <button onClick={()=>{downloadSchoolExcel("grades");setShowXl(false);}} style={{width:"100%",display:"block",padding:"8px 14px",background:"none",border:"none",cursor:"pointer",textAlign:"left",fontSize:12,fontFamily:F,color:"#374151",borderRadius:8}} onMouseEnter={e=>e.target.style.background="#f0fdf4"} onMouseLeave={e=>e.target.style.background="none"}>🎯 All Classes — Grades (Excel)</button>
            </div>}
          </div>
  ...
  function getSubTeacher(subject){
    ...
    if(parts){
      return parts.map(p=>{
        ...
        if(tid2){const names=Array.isArray(tid2)?tid2:[tid2];return names.map(n=>{const u=(users||[]).find(u=>u.id===n);return u?u.name:n;}).join(", ");}
    ...
    if(tid){const names=Array.isArray(tid)?tid:[tid];return names.map(n=>{const u=(users||[]).find(u=>u.id===n);return u?u.name:n;});}
    const cls=(sd.classes||[]).find(c=>c.name===student.class);
    ...
    <Card style={{maxWidth:720,margin:"0 auto",fontFamily:F}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr auto",gap:16,marginBottom:16}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,fontSize:12}}>
          ...
          <div><b>Position:</b> <span style={{fontWeight:"bold",color:"#3b0764"}}>{position} / {classStudents.length}</span></div>
      ...
      <div style={{overflowX:"auto",WebkitOverflowScrolling:"touch"}}><table style={{width:"100%",borderCollapse:"collapse",marginBottom:14,minWidth:500}}>
        <thead><tr style={{background:"#3b0764",color:"white"}}>{["Subject","Marks","Grade","Points","Remarks","Init."].map(h=><th key={h} style={{padding:"7px 10px",textAlign:"left",fontSize:11}}>{h}</th>)}</tr></thead>
        <tbody>
          {subs.map((s,i)=>{
            ...
            return(<tr key={s} style={{background:i%2===0?"white":"#fafafa"}}>
              ...
              <td style={{padding:"6px 10px",fontSize:11,color:"#3b0764",textAlign:"center",fontWeight:"bold"}} title={teacherTitle}>{initials}</td>
            </tr>);
          })}
        </tbody>
        <tfoot>
      ...
      <div style={{border:"1px solid #e2e8f0",borderRadius:8,padding:12,marginBottom:14}}><div style={{fontSize:12,fontWeight:"bold",color:"#3b0764",marginBottom:6}}>Class Teacher's Comment</div><div style={{fontSize:12,color:"#374151",minHeight:36}}>{comment?comment.text:<i style={{color:"#94a3b8"}}>No comment recorded.</i>}</div>{comment&&<div style={{fontSize:10,color:"#94a3b8",marginTop:6}}>— {comment.teacher} ({comment.date})</div>}</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:16,marginTop:14}}>
        <div style={{textAlign:"center"}}><div style={{borderTop:"1px solid #374151",marginBottom:4,paddingTop:4,fontSize:11,color:"#64748b"}}>Class Teacher</div>{clsTeacher&&<div style={{fontSize:11,color:"#3b0764",fontWeight:"bold"}}>{clsTeacher}</div>}<div style={{fontSize:9,color:"#94a3b8",marginTop:8}}>Signature & Date</div></div>
...
function FeesPage({students,fees,setFees,user,logo}) {
  ...
  function printStatement(title,rows,totals){
    const tableHTML=`<h3 style="margin:0 0 12px;color:#3b0764;">${title}</h3>
  ...
  function buildStudentFeeHTML(student) {
    ...
    const rows=sFees.map((f,i)=>{
      ...
      return `<tr style="background:${i%2===0?"rgba(255,255,255,0.5)":"rgba(248,250,252,0.5)"}">
    ...
    return `<div style="page-break-after:always;padding:20px 24px;max-width:700px;margin:0 auto;">
      ${buildSectionHeader(logo)}
      <div style="background:#3b0764;color:white;text-align:center;padding:6px 0;font-size:12px;font-weight:bold;border-radius:16px;margin-bottom:14px;letter-spacing:1px;">FEE STATEMENT</div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:11px;margin-bottom:14px;border:1px solid #e2e8f0;border-radius:6px;padding:10px;background:rgba(255,255,255,0.6);">
        <div><b>Name:</b> ${student.name}</div><div><b>Adm. No:</b> ${student.admNo||"—"}</div>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
          ...
          {user.role==="admin"&&<div style={{position:"relative"}}>
            ...
            {showDlMenu&&<div style={{position:"absolute",top:"100%",right:0,zIndex:200,background:"white",border:"1px solid #e2e8f0",borderRadius:12,boxShadow:"0 8px 24px rgba(0,0,0,.12)",padding:8,minWidth:260,marginTop:4,maxHeight:400,overflowY:"auto"}}>
              ...
              <button onClick={()=>{printSchoolFee();setShowDlMenu(false);}} style={{width:"100%",display:"block",padding:"8px 14px",background:"#3b0764",border:"none",cursor:"pointer",textAlign:"left",fontSize:12,fontFamily:F,color:"white",borderRadius:8,fontWeight:"bold",marginBottom:4}}>🏫 All Learners — Full School</button>
            </div>}
          </div>}
        </div>
      </PageH>
                ...
                return(
                  <tr key={f.id} style={{background:i%2===0?"white":"#fafafa"}}>
                    <td style={{...td,fontWeight:"bold"}}>
                      <button onClick={()=>setStuModal(s)} style={{background:"none",border:"none",color:"#7c3aed",cursor:"pointer",fontWeight:"bold",fontFamily:F,fontSize:12,padding:0,textDecoration:"underline"}}>{s?.name||"—"}</button>
                    </td>
            ...
            <tbody>
              {stuSummary.length?stuSummary.map((x,i)=>(
                <tr key={x.student.id} style={{background:i%2===0?"white":"#fafafa"}}>
                  ...
                  <td style={{...td,fontWeight:"bold"}}>
                    <button onClick={()=>setStuModal(x.student)} style={{background:"none",border:"none",color:"#7c3aed",cursor:"pointer",fontWeight:"bold",fontFamily:F,fontSize:12,padding:0,textDecoration:"underline"}}>{x.student.name}</button>
                  </td>
      ...
      {stuModal&&(()=>{
        ...
        return(
          <Modal title={`📋 Fee Statement — ${stuModal.name}`} onClose={()=>setStuModal(null)} wide>
            ...
            <div style={{overflowX:"auto",WebkitOverflowScrolling:"touch"}}><table style={{width:"100%",borderCollapse:"collapse",minWidth:600}}>
              <thead><tr>{["Fee Type","Term","Year","Due","Paid","Balance ⚡","Method","Receipt","Date"].map(h=><th key={h} style={{textAlign:"left",padding:"8px 10px",fontWeight:"bold",fontSize:11,color:"#7c3aed",background:"#f5f3ff"}}>{h}</th>)}</tr></thead>
              <tbody>
...
function TimetableSetup({staff,setupData,setSetupData}) {
  ...
  return (
    <div>
      ...
      {tab==="classes"&&<>
        <Card style={{marginBottom:16}}>
          <div style={{fontWeight:"bold",color:"#3b0764",marginBottom:14,fontSize:14}}>➕ Add Class</div>
        ...
        <Card style={{padding:0}}>
          <div style={{padding:"12px 16px",background:"#f5f3ff",fontWeight:"bold",color:"#3b0764",fontSize:13,borderBottom:"1px solid #ede9fe"}}>Configured Classes ({classes.length})</div>
          <div style={{overflowX:"auto",WebkitOverflowScrolling:"touch"}}><table style={{width:"100%",borderCollapse:"collapse",minWidth:600}}>
            ...
            <tbody>
              {classes.length?classes.map((c,i)=>(
                <tr key={c.id} style={{background:i%2===0?"white":"#fafafa"}}>
                  ...
                  <td style={td}><span style={{background:"#f5f3ff",color:"#7c3aed",fontSize:11,padding:"2px 8px",borderRadius:20,fontWeight:"bold"}}>{(c.subjects||[]).length} subjects</span></td>
      ...
      {tab==="subjects"&&<>
        <Card style={{marginBottom:14}}>
          <div style={{fontWeight:"bold",color:"#3b0764",marginBottom:10,fontSize:14}}>Select Class to Configure</div>
        ...
        {selClass&&<>
          <Card style={{marginBottom:14}}>
            <div style={{fontWeight:"bold",color:"#3b0764",marginBottom:10,fontSize:14}}>➕ Add Subject to {selClass.name}{selClass.stream?` (${selClass.stream})`:""}</div>
          ...
          <Card style={{padding:0}}>
            <div style={{padding:"12px 16px",background:"#f5f3ff",fontWeight:"bold",color:"#3b0764",fontSize:13,borderBottom:"1px solid #ede9fe"}}>{selClass.name}{selClass.stream?` (${selClass.stream})`:""} — Subjects & Teachers</div>
            <div style={{overflowX:"auto",WebkitOverflowScrolling:"touch"}}><table style={{width:"100%",borderCollapse:"collapse",minWidth:600}}>
          ...
          {classes.map(c=>{
            ...
            return (
              <Card key={c.id} style={{borderLeft:`4px solid ${ct?"#1d4ed8":"#e2e8f0"}`}}>
                ...
                <div style={{marginTop:10,display:"flex",gap:6,flexWrap:"wrap"}}>
                  {(c.subjects||[]).map((s,i)=>(
                    <div key={i} style={{background:s.teacherName?"#f5f3ff":"#fef2f2",border:`1px solid ${s.teacherName?"#ddd6fe":"#fecaca"}`,borderRadius:8,padding:"4px 10px",fontSize:11}}>
                      <div style={{fontWeight:"bold",color:"#3b0764"}}>{s.subject}</div>
                      <div style={{color:s.teacherName?"#7c3aed":"#b91c1c"}}>{s.teacherName||"⚠️ No teacher"}</div>
                    </div>
                  ))}
                </div>
              </Card>
...
function TimetablePage({students, staff, user, timetable:tt, setTimetable:setTt, ttSetup, setTtSetup, logo}) {
  ...
  const genStopRef                      = useRef(false);   // set to true to stop Stage 2
  ...
  function buildTTHeader(title, subtitle) {
    ...
    const logoTag = logoSrc
      ...
      : '';
    return `
      <div style="text-align:center;margin-bottom:14px;border-bottom:2px solid #3b0764;padding-bottom:10px;">
        ${logoTag}
  ...
  function buildTTTable(headers, rows, colColors) {
    ...
    const headRow = headers.map((h,i)=>`<th style="${thStyle}background:${i===0?"#3b0764":"#7c3aed"};color:white;">${h}</th>`).join("");
  ...
  function ttPrintOverlay(title, bodyHTML) {
    ...
    // Toolbar
    const toolbar = document.createElement("div");
    toolbar.style.cssText = "display:flex;align-items:center;justify-content:space-between;padding:10px 16px;background:#3b0764;flex-shrink:0;gap:10px;";
  ...
  function buildClassTimetableHTML(cls) {
    ...
    const allRows = MAIN_SLOTS.map((slot)=>{
      ...
      const cells = days.map(day => {
        ...
        const bg = globalColMap[cell.subject]||"#f5f3ff";
        ...
        const inner = isPPI
          ...
          : `<b style="color:#3b0764;">${getShort(cell.subject)}</b><br/>`+
      ...
      return `<tr><td style="padding:6px 8px;border:1px solid #e2e8f0;background:#f8fafc;white-space:nowrap;"><b style="font-size:11px;color:#3b0764;">${slot.name||slot.label}</b><br/><span style="font-size:9px;color:#94a3b8;">${slot.start}–${slot.end}</span></td>${cells}</tr>`;
    }).join("");
    const tableHTML = `<table style="width:100%;border-collapse:collapse;font-size:11px;">
      <thead><tr>${["Period / Time",...days].map((h,i)=>`<th style="padding:8px;background:${i===0?"#3b0764":"#7c3aed"};color:white;text-align:center;font-size:11px;">${h}</th>`).join("")}</tr></thead>
      <tbody>${allRows}</tbody>
    </table>`;
  ...
  function buildTeacherTimetableHTML(teacher) {
    ...
    const allRows = MAIN_SLOTS.map((slot) => {
      ...
      const dayCols = days.map(day=>{
        ...
        const cell = (tt[cls]?.[day]||[])[si];
        const bg = globalColMap[cell?.subject]||"#f5f3ff";
        return `<td style="padding:6px 8px;border:1px solid #e2e8f0;background:${bg};text-align:center;vertical-align:top;"><b style="font-size:10px;color:#3b0764;">${getShort(cell.subject)}</b><br/><span style="font-size:9px;color:#374151;">${cls}</span>${cell.double?`<br/><span style="font-size:8px;color:#7c3aed;font-weight:bold;">2P</span>`:""}</td>`;
      }).join("");
      return `<tr><td style="padding:6px 8px;border:1px solid #e2e8f0;background:#f8fafc;white-space:nowrap;"><b style="font-size:11px;color:#3b0764;">${slot.name||slot.label}</b><br/><span style="font-size:9px;color:#94a3b8;">${slot.start}–${slot.end}</span></td>${dayCols}</tr>`;
    }).join("");
  ...
  // ── Conflict detection ────────────────────────────────────────────────────
  // ══════════════════════════════════════════════════════════════════════════
  // aSc TIMETABLES ENGINE
  // Rebuilt from scratch following aSc Timetables' published methodology:
  //
  // CORE ALGORITHM: Constraint Satisfaction with Backtracking + Tabu Search
  //
  // Phase 1 — VARIABLE ORDERING (Most-Constrained-First):
  //   Sort every (class, subject) lesson by how few valid slots exist for it.
  //   Most constrained = fewest teacher-free AND avail-matching slots.
  //   These are placed first — just like aSc does with "difficult lessons first".
  //
  // Phase 2 — CONSTRAINT PROPAGATION (Arc Consistency):
  //   After placing each lesson, immediately update the domain of all remaining
  //   lessons that share the same teacher — forward-checking prevents dead ends.
  //
  // Phase 3 — PLACEMENT WITH LOOK-AHEAD:
  //   For each lesson, try every valid slot. After each tentative placement,
  //   check that at least one valid slot still exists for every remaining lesson
  //   of the same teacher. If not, backtrack immediately.
  //
  // Phase 4 — TABU SEARCH REPAIR (aSc's "intelligent conflict resolution"):
  //   Once Phase 3 is done, any remaining conflicts are resolved by Tabu Search:
  //   - Maintain a "tabu list" of recently tried swaps (prevents cycling)
  //   - Each iteration: find the worst conflict, try all neighbour swaps,
  //     pick the best non-tabu move even if it temporarily worsens score
  //   - This escapes local minima far better than random restarts
  //
  // Each phase runs in its own setTimeout tick — ZERO main-thread blocking.
  // ══════════════════════════════════════════════════════════════════════════

  // ── Conflict detector (used by both render and engine) ────────────────────
      ...
      LESSON_SLOTS.forEach((slot, si) => {
        ...
        ALL_CLASSES.forEach(cls => {
          ...
          if(cell?.teacher && cell.teacher !== "TBD" &&
             !cell.teacher.includes("All Staff"))
        ...
        Object.entries(seen).forEach(([t, classes]) => {
          if(classes.length > 1) {
            classes.forEach(cls => {
              c[`${cls}-${day}-${slot.period}`] =
                `${t} also in ${classes.filter(x=>x!==cls).join(",")}`;
            });
          }
        });
      });
  ...
  // ── Valid 2P double-lesson pairs: only P1+P2, P3+P4, P5+P6, P7+P8 ────────
  // (i.e. odd-numbered period paired with the next even-numbered period)
  // These are pairs of LESSON_SLOT indices where both periods are truly
  // adjacent with NO break between them in the bell schedule.
  function getConsecPairs() {
    const pairs = [];
    // Walk the full bell schedule. A valid double pair = two period entries
    // that appear back-to-back with NO break between them.
    for(let i = 0; i < bellPeriods.length - 1; i++) {
      if(bellPeriods[i].type !== "period") continue;
      if(bellPeriods[i+1].type !== "period") continue; // break between them — skip
    ...
    // Fallback: if nothing found, pair up even-odd indices
    if(pairs.length === 0) {
      for(let i = 0; i < LESSON_SLOTS.length - 1; i += 2) pairs.push([i, i+1]);
    }
    return pairs;
  }
  ...
  // ── Auto-scale plan so LPW always totals to exactly available slots ───────
      ...
      if(rawTotal === 0) {
        ...
        sorted.forEach((sub, i) => {
          if(i === sorted.length - 1) {
            ...
            const v = Math.min(
              remaining - (sorted.length - 1 - i),
              Math.max(1, Math.round(rawLpw[sub] * scale))
            );
            scaledLpw[sub] = v;
            remaining -= v;
          }
  ...
  function autoGen() {
    if(LESSON_SLOTS.length < 1) {
      flash("⚠️ No periods in bell schedule. Go to Setup → Bell Schedule.", "warn"); return;
    }
  ...
  async function aiGenerate() {
    ...
    try {
      ...
      // Compact class data - drop teachers/bell to reduce token usage
...
DATA:${JSON.stringify(compactSetups)}`;
      ...
      const rawText = await callGroq(prompt, 4000);
      setGenProgress(65);
  ...
  function TimetableCell({cls, day, slotIdx, cell, isAdmin}) {
    ...
    const bg     = isPPI ? "#fef3c7" : hasConflict ? "#fee2e2" : cell?.subject ? (globalColMap[cell.subject]||"#f5f3ff") : "#f8fafc";
      ...
      >
        ...
        >
          {isPPI ? (
            ...
            <>
              <div style={{fontSize:10,fontWeight:"bold",color:"#3b0764",textAlign:"center",lineHeight:1.2}}>{short}</div>
      ...
      <tr style={{background:isEdit?"#fffbeb":bp.type==="break"?"#fefce8":"white", borderTop:"1px solid #f1f5f9"}}>
        ...
        <td style={{padding:"7px 10px"}}>
          <span style={{fontSize:10,padding:"2px 8px",borderRadius:16,fontWeight:"bold",
            background:bp.type==="break"?"#fef3c7":"#f5f3ff",
            color:bp.type==="break"?"#b45309":"#7c3aed"}}>
        ...
        <td style={{padding:"7px 10px"}}>
          {isEdit ? (
            <input value={bp.name} onChange={e=>updateBellEntry(bp.id,"name",e.target.value)}
              style={{border:"1.5px solid #c4b5fd",borderRadius:6,padding:"4px 8px",fontSize:12,fontFamily:FT,outline:"none",width:160}}/>
          ) : <span style={{fontSize:12,fontWeight:"bold",color:"#3b0764"}}>{bp.name}</span>}
        </td>
        <td style={{padding:"7px 10px"}}>
          {isEdit ? (
            <input type="time" value={bp.start} onChange={e=>updateBellEntry(bp.id,"start",e.target.value)}
              style={{border:"1.5px solid #c4b5fd",borderRadius:6,padding:"4px 8px",fontSize:12,outline:"none"}}/>
  ...
  return (
    <div style={{padding:24,fontFamily:FT}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20,flexWrap:"wrap",gap:10}}>
        <div>
          <h2 style={{margin:0,fontSize:22,color:"#3b0764",fontFamily:FT}}>
            📅 TimetableMaster
          </h2>
          ...
          {isAdmin && (
            <>
              <button onClick={()=>setShowSaveModal(true)}
                style={{background:"#f5f3ff",color:"#7c3aed",border:"1px solid #ddd6fe",borderRadius:10,padding:"8px 16px",cursor:"pointer",fontFamily:FT,fontSize:13,fontWeight:"bold"}}>
                💾 Save
              </button>
        ...
        <div style={{marginBottom:14,background:"#f5f3ff",border:"1px solid #ddd6fe",borderRadius:12,padding:"12px 16px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
            ...
            {!aiMode && genProgress >= 20 && (
              <button onClick={()=>{ genStopRef.current = true; }}
                style={{background:"#fee2e2",color:"#b91c1c",border:"1px solid #fca5a5",borderRadius:7,padding:"4px 12px",cursor:"pointer",fontSize:12,fontWeight:"bold",fontFamily:FT}}>
                ⏹ Stop &amp; Accept Best
              </button>
            )}
          </div>
          ...
          <div style={{fontSize:11,color:"#1e3a8a",marginTop:6}}>
            {aiMode
              ...
              : genProgress < 20
                ? "Stage 1 fills every slot instantly. Stage 2 then runs until zero conflicts."
                : `Stage 2 runs until every teacher conflict is resolved. Click "Stop & Accept Best" anytime once you're happy with the result.`
            }
          </div>
        </div>
      )}
      ...
      <div style={{display:"flex",gap:4,marginBottom:18,background:"#f1f5f9",borderRadius:12,padding:4,flexWrap:"wrap"}}>
        {[["setup","🗂️ Setup"],["bell","🔔 Bell"],["dashboard","📊 Dashboard"],["view","📅 Timetable"],["subjects","⚙️ Subjects"],["teacher","👤 Teachers"],["daily","☀️ Full Day"],["weekend","📆 Weekend"],["saved","💾 Saved"]].map(([t,l])=>(
          <button key={t} onClick={()=>setTab(t)} style={{flex:1,padding:"7px 10px",border:"none",borderRadius:9,cursor:"pointer",fontFamily:FT,fontSize:12,fontWeight:tab===t?"bold":"normal",background:tab===t?"white":"transparent",color:tab===t?"#3b0764":"#64748b",boxShadow:tab===t?"0 1px 4px rgba(0,0,0,.1)":"none",transition:"all .15s",minWidth:80,whiteSpace:"nowrap"}}>
            {l}
          </button>
        ))}
      </div>
      ...
      {tab==="setup" && (
        <div style={{display:"grid",gap:16}}>
          ...
          <div style={{background:"white",borderRadius:14,padding:"16px 20px",boxShadow:"0 2px 12px rgba(0,0,0,.07)"}}>
            <div style={{fontWeight:"bold",color:"#3b0764",fontSize:14,marginBottom:14}}>📋 Setup — Step {setupStep} of 9</div>
          ...
          {setupStep===1 && (
            <div style={{background:"white",borderRadius:14,padding:20,boxShadow:"0 2px 12px rgba(0,0,0,.07)"}}>
              <div style={{fontWeight:"bold",color:"#3b0764",fontSize:15,marginBottom:12}}>Step 1 — Timetable Information</div>
              ...
              <button onClick={()=>setSetupStep(2)} style={{background:"linear-gradient(135deg,#7c3aed,#3b0764)",color:"white",border:"none",borderRadius:9,padding:"9px 20px",cursor:"pointer",fontFamily:FT,fontSize:13,fontWeight:"bold"}}>Save & Continue →</button>
            </div>
          )}
          ...
          {setupStep===2 && (
            <div style={{background:"white",borderRadius:14,padding:20,boxShadow:"0 2px 12px rgba(0,0,0,.07)"}}>
              <div style={{fontWeight:"bold",color:"#3b0764",fontSize:15,marginBottom:8}}>Step 2 — Bell Schedule</div>
                ...
                {["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"].map(d=>(
                  <button key={d} onClick={()=>{if(isAdmin)setWorkingDays(prev=>prev.includes(d)?prev.filter(x=>x!==d):[...prev,d])}}
                    style={{padding:"6px 14px",border:"none",borderRadius:8,cursor:isAdmin?"pointer":"default",fontFamily:FT,fontSize:12,fontWeight:"bold",
                      background:workingDays.includes(d)?"linear-gradient(135deg,#7c3aed,#3b0764)":"#f1f5f9",
                      color:workingDays.includes(d)?"white":"#64748b"}}>
                    {d.slice(0,3)}
                  </button>
                ))}
              </div>
              <button onClick={()=>setTab("bell")} style={{background:"#f5f3ff",color:"#7c3aed",border:"1px solid #ddd6fe",borderRadius:9,padding:"8px 16px",cursor:"pointer",fontFamily:FT,fontSize:13,fontWeight:"bold",marginRight:8}}>Open Bell Editor →</button>
              <div style={{display:"flex",gap:8,marginTop:12}}>
                ...
                <button onClick={()=>setSetupStep(3)} style={{background:"linear-gradient(135deg,#7c3aed,#3b0764)",color:"white",border:"none",borderRadius:9,padding:"9px 20px",cursor:"pointer",fontFamily:FT,fontSize:13,fontWeight:"bold"}}>Continue →</button>
              </div>
            </div>
          )}
          ...
          {(setupStep>=3 && setupStep<=7) && (
            <div style={{background:"white",borderRadius:14,padding:20,boxShadow:"0 2px 12px rgba(0,0,0,.07)"}}>
              <div style={{fontWeight:"bold",color:"#3b0764",fontSize:15,marginBottom:8}}>
              ...
              {(setupStep===3||setupStep===6||setupStep===7) && (
                <button onClick={()=>setTab("subjects")} style={{background:"#f5f3ff",color:"#7c3aed",border:"1px solid #ddd6fe",borderRadius:9,padding:"8px 16px",cursor:"pointer",fontFamily:FT,fontSize:13,fontWeight:"bold",marginBottom:12,display:"inline-block"}}>
              ...
              <div style={{display:"flex",gap:8}}>
                ...
                <button onClick={()=>setSetupStep(setupStep+1)} style={{background:"linear-gradient(135deg,#7c3aed,#3b0764)",color:"white",border:"none",borderRadius:9,padding:"9px 20px",cursor:"pointer",fontFamily:FT,fontSize:13,fontWeight:"bold"}}>Continue →</button>
              </div>
            </div>
          )}
          ...
          {setupStep===8 && (
            <div style={{background:"white",borderRadius:14,padding:20,boxShadow:"0 2px 12px rgba(0,0,0,.07)"}}>
              <div style={{fontWeight:"bold",color:"#3b0764",fontSize:15,marginBottom:8}}>Step 8 — Review & Generate</div>
              ...
              <div style={{marginTop:12,display:"flex",gap:8}}>
                ...
                <button onClick={()=>setSetupStep(9)} style={{background:"linear-gradient(135deg,#7c3aed,#3b0764)",color:"white",border:"none",borderRadius:9,padding:"9px 20px",cursor:"pointer",fontFamily:FT,fontSize:13,fontWeight:"bold"}}>Next →</button>
              </div>
            </div>
          )}
          ...
          {setupStep===9 && (
            <div style={{background:"white",borderRadius:14,padding:20,boxShadow:"0 2px 12px rgba(0,0,0,.07)"}}>
              <div style={{fontWeight:"bold",color:"#3b0764",fontSize:15,marginBottom:8}}>Step 9 — Fine-Tune & Share</div>
              <div style={{display:"grid",gap:10}}>
                {[
                  ...
                  <div key={item.title} style={{display:"flex",alignItems:"center",gap:14,padding:"12px 16px",background:"#f8fafc",borderRadius:10,border:"1px solid #e2e8f0"}}>
                    ...
                    <div style={{flex:1}}>
                      <div style={{fontWeight:"bold",fontSize:13,color:"#3b0764"}}>{item.title}</div>
        ...
        <div style={{display:"grid",gap:16}}>
          <div style={{background:"white",borderRadius:14,padding:20,boxShadow:"0 2px 12px rgba(0,0,0,.07)"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16,flexWrap:"wrap",gap:8}}>
              <div>
                <div style={{fontWeight:"bold",color:"#3b0764",fontSize:15}}>🔔 Bell Schedule Editor</div>
              ...
              {isAdmin && (
                <div style={{display:"flex",gap:8}}>
                  <button onClick={()=>addBellEntry("period")} style={{background:"linear-gradient(135deg,#7c3aed,#3b0764)",color:"white",border:"none",borderRadius:8,padding:"7px 14px",cursor:"pointer",fontFamily:FT,fontSize:12,fontWeight:"bold"}}>+ Period</button>
                ...
                <thead>
                  <tr style={{background:"#f5f3ff"}}>
                    {["#","Type","Name","Start","End","Actions"].map(h=>(
                      <th key={h} style={{padding:"9px 12px",fontSize:11,textAlign:"left",color:"#7c3aed",fontWeight:"bold"}}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
            ...
            <div style={{marginTop:16,padding:"12px 14px",background:"#f8fafc",borderRadius:10,border:"1px solid #e2e8f0"}}>
              ...
              <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                {MAIN_SLOTS.map((s,i)=>(
                  <div key={i} style={{background:s.isBreak?"#fef3c7":"#f5f3ff",border:`1px solid ${s.isBreak?"#fde68a":"#ddd6fe"}`,borderRadius:8,padding:"5px 10px",fontSize:11}}>
                    <div style={{fontWeight:"bold",color:s.isBreak?"#b45309":"#7c3aed"}}>{s.name}</div>
        ...
        <div style={{display:"grid",gap:16}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
            <div style={{background:"white",borderRadius:14,padding:18,boxShadow:"0 2px 12px rgba(0,0,0,.07)"}}>
              <div style={{fontWeight:"bold",color:"#3b0764",fontSize:14,marginBottom:14}}>👤 Teacher Workload</div>
          ...
          <div style={{background:"white",borderRadius:14,padding:18,boxShadow:"0 2px 12px rgba(0,0,0,.07)"}}>
            <div style={{fontWeight:"bold",color:"#3b0764",fontSize:14,marginBottom:14}}>📚 CBC Weekly Lesson Allocation</div>
              ...
              {ALL_CLASSES.map(cls=>{
                ...
                return (
                  <div key={cls} style={{background:"#f8fafc",borderRadius:10,padding:"10px 14px",border:"1px solid #e2e8f0"}}>
                    <div style={{fontWeight:"bold",color:"#3b0764",fontSize:12,marginBottom:6}}>{cls} · {total} lessons/wk</div>
                      ...
                      {subs.map(sub=>{
                        ...
                        return (
                          <div key={sub} style={{background:dbl?"#f3e8ff":"white",border:`1px solid ${dbl?"#a855f7":"#e2e8f0"}`,borderRadius:5,padding:"2px 7px",fontSize:10,display:"flex",gap:3,alignItems:"center"}}>
                            <span style={{fontWeight:"bold",color:dbl?"#7c3aed":"#7c3aed"}}>{n}×</span>
      ...
      {tab==="view" && (
        <>
          <div style={{display:"flex",gap:10,marginBottom:14,flexWrap:"wrap",alignItems:"center"}}>
            <select value={selCls} onChange={e=>setSelCls(e.target.value)} style={{border:"1.5px solid #e2e8f0",borderRadius:8,padding:"8px 12px",fontSize:13,fontFamily:FT,fontWeight:"bold",color:"#3b0764"}}>
              {ALL_CLASSES.map(c=><option key={c} value={c}>{c}</option>)}
            </select>
            <div style={{display:"flex",background:"#f1f5f9",borderRadius:8,padding:3,gap:2}}>
              {[["Mon–Fri",false],["Weekend",true]].map(([l,k])=>(
                <button key={l} onClick={()=>setWeekendView(k)} style={{padding:"6px 14px",border:"none",borderRadius:6,cursor:"pointer",fontFamily:FT,fontSize:12,fontWeight:weekendView===k?"bold":"normal",background:weekendView===k?"white":"transparent",color:weekendView===k?"#3b0764":"#64748b"}}>
                  {l}
                </button>
              ))}
            </div>
            ...
            <button onClick={()=>printClassTimetable(selCls)}
              style={{background:"linear-gradient(135deg,#3b0764,#7c3aed)",color:"white",border:"none",borderRadius:8,padding:"7px 14px",cursor:"pointer",fontFamily:FT,fontSize:12,fontWeight:"bold",display:"flex",alignItems:"center",gap:5}}>
              🖨️ Print This Class
            </button>
            <button onClick={printAllClassTimetables}
              style={{background:"#f5f3ff",color:"#7c3aed",border:"1px solid #ddd6fe",borderRadius:8,padding:"7px 14px",cursor:"pointer",fontFamily:FT,fontSize:12,fontWeight:"bold"}}>
              🖨️ All Classes
            </button>
          </div>
          ...
          <div style={{background:"white",borderRadius:14,boxShadow:"0 2px 12px rgba(0,0,0,.08)",overflow:"hidden"}}>
                  ...
                  {(weekendView ? SAT_LESSON_SLOTS : MAIN_SLOTS).map((slot,i)=>{
                    ...
                    return (
                      <tr key={`p${slot.period}`} style={{borderTop:"1px solid #f1f5f9"}}>
                        <td style={{padding:"6px 14px",fontWeight:"bold",color:"#3b0764",fontSize:12,whiteSpace:"nowrap"}}>{slot.name||slot.label}</td>
            ...
            <div style={{padding:"10px 16px",borderTop:"1px solid #f1f5f9",display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>
              <span style={{fontSize:11,fontWeight:"bold",color:"#3b0764"}}>Legend:</span>
              {getTTSubs(selCls).map(s=>(
                <span key={s} style={{fontSize:10,background:clsColMap[s]||"#f8fafc",padding:"2px 7px",borderRadius:20,color:"#3b0764",fontWeight:"bold",border:"1px solid rgba(0,0,0,.08)"}}>
            ...
            <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:9000,padding:16}}>
              <div style={{background:"white",borderRadius:18,padding:24,width:"95%",maxWidth:480,maxHeight:"90vh",overflowY:"auto",fontFamily:FT}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
                  <div style={{fontWeight:"bold",color:"#3b0764",fontSize:15}}>✏️ Edit Period — {editCell.cls} · {editCell.day} P{editCell.p+1}</div>
      ...
      {tab==="subjects" && (
        <div style={{display:"grid",gap:16}}>
          <div style={{background:"#f5f3ff",border:"1px solid #ddd6fe",borderRadius:10,padding:"12px 16px",fontSize:12,color:"#7c3aed",lineHeight:1.8}}>
            ...
            {[["byclass","🏫 By Class"],["bysubject","📚 By Subject"],["byteacher","👤 By Teacher"]].map(([v,l])=>(
              <button key={v} onClick={()=>setSubView(v)}
                style={{padding:"7px 16px",border:"none",borderRadius:8,cursor:"pointer",fontFamily:FT,fontSize:12,fontWeight:"bold",
                  background:subView===v?"linear-gradient(135deg,#3b0764,#7c3aed)":"#f1f5f9",
            ...
            return allSubjects.map(sub=>{
              ...
              return (
                <div key={sub} style={{background:"white",borderRadius:14,boxShadow:"0 2px 12px rgba(0,0,0,.07)",overflow:"hidden"}}>
                  <div style={{padding:"12px 18px",background:"linear-gradient(135deg,#3b0764,#7c3aed)",color:"white",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <div>
                    ...
                    {classes.map(cls=>{
                      ...
                      return (
                        <div key={cls} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 10px",background:"#f8fafc",borderRadius:8,flexWrap:"wrap"}}>
                          <span style={{fontWeight:"bold",fontSize:12,color:"#3b0764",minWidth:80}}>{cls}</span>
            ...
            return (
              <div key={teacher} style={{background:"white",borderRadius:14,boxShadow:"0 2px 12px rgba(0,0,0,.07)",overflow:"hidden",border:`2px solid ${over?"#fecaca":"#e2e8f0"}`}}>
                <div style={{padding:"12px 18px",background:over?"#fef2f2":"#f5f3ff",display:"flex",alignItems:"center",gap:12}}>
                  ...
                  <div>
                    <div style={{fontWeight:"bold",color:"#3b0764",fontSize:14}}>{teacher}</div>
                ...
                <div style={{padding:"10px 18px",display:"flex",gap:8,flexWrap:"wrap"}}>
                  {lessons.length ? lessons.map((l,i)=>(
                    <div key={i} style={{background:"#f5f3ff",borderRadius:8,padding:"6px 12px",fontSize:11}}>
                      <div style={{fontWeight:"bold",color:"#3b0764"}}>{l.cls}</div>
      ...
      {tab==="teacher" && (
        <div style={{display:"grid",gap:16}}>
          <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
            <span style={{fontSize:13,fontWeight:"bold",color:"#3b0764"}}>Filter:</span>
            ...
            {filterTeacher!=="All" && (
              <button onClick={()=>printTeacherTimetable(filterTeacher)}
                style={{background:"linear-gradient(135deg,#3b0764,#7c3aed)",color:"white",border:"none",borderRadius:8,padding:"7px 14px",cursor:"pointer",fontFamily:FT,fontSize:12,fontWeight:"bold"}}>
              ...
              <div key={teacher} style={{background:"white",borderRadius:14,boxShadow:"0 2px 12px rgba(0,0,0,.07)",overflow:"hidden"}}>
                <div style={{padding:"12px 18px",background:over?"linear-gradient(135deg,#fee2e2,#fef2f2)":"linear-gradient(135deg,#f5f3ff,#ede9fe)",display:"flex",alignItems:"center",gap:12,borderBottom:"1px solid #f1f5f9",flexWrap:"wrap"}}>
                  ...
                  <button onClick={()=>printTeacherTimetable(teacher)}
                    style={{background:"linear-gradient(135deg,#3b0764,#7c3aed)",color:"white",border:"none",borderRadius:8,padding:"6px 14px",cursor:"pointer",fontFamily:FT,fontSize:11,fontWeight:"bold",whiteSpace:"nowrap"}}>
                    🖨️ Print
                  </button>
                </div>
                <div style={{overflowX:"auto"}}>
                      ...
                      {LESSON_SLOTS.map((slot,si)=>(
                        <tr key={slot.period} style={{borderTop:"1px solid #f1f5f9"}}>
                          ...
                          {DAYS.map(day=>{
                            ...
                            const bg   = cell?.subject ? (globalColMap[cell.subject]||"#f5f3ff") : "#f8fafc";
                            return (
                              <td key={day} style={{padding:4}}>
                                {cls ? (
                                  <div style={{background:bg,borderRadius:7,padding:"5px 6px",textAlign:"center",fontSize:10}}>
                                    <div style={{fontWeight:"bold",color:"#3b0764"}}>{getShort(cell.subject)}</div>
      ...
      {tab==="daily" && (
        <div style={{display:"grid",gap:10}}>
          <div style={{background:"#f5f3ff",border:"1px solid #ddd6fe",borderRadius:8,padding:"10px 14px",fontSize:12,color:"#7c3aed"}}>
            ℹ️ Full boarding school daily routine. Admins can click a row to edit activity descriptions.
          </div>
          {daySchedule.map((item,i)=>(
            <div key={i} style={{background:"white",borderRadius:12,padding:"12px 18px",boxShadow:"0 1px 6px rgba(0,0,0,.06)",borderLeft:`4px solid ${item.editable?"#b45309":"#7c3aed"}`,display:"flex",alignItems:"center",gap:14}}>
              ...
              {isAdmin && item.editable && editSchedIdx!==i && (
                <button onClick={()=>setEditSchedIdx(i)} style={{background:"#f5f3ff",color:"#7c3aed",border:"none",borderRadius:8,padding:"6px 12px",cursor:"pointer",fontSize:11,fontWeight:"bold",whiteSpace:"nowrap"}}>Edit</button>
              )}
            </div>
          ))}
        </div>
            ...
            {["Saturday","Sunday"].map(d=>(
              <button key={d} onClick={()=>setWeekendEditCell(d)}
                style={{padding:"8px 20px",border:"none",borderRadius:9,cursor:"pointer",fontFamily:FT,fontSize:13,fontWeight:"bold",
                  background:weekendEditCell===d||(!weekendEditCell&&d==="Saturday")?"linear-gradient(135deg,#7c3aed,#3b0764)":"#f1f5f9",
                  ...
                  {weekendEditVal[`${day}-${i}`] !== undefined ? (
                    <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
                      <input value={weekendEditVal[`${day}-${i}`].time||item.time} onChange={e=>setWeekendEditVal(v=>({...v,[`${day}-${i}`]:{...(v[`${day}-${i}`]||{}),time:e.target.value}}))}
                        style={{border:"1.5px solid #c4b5fd",borderRadius:6,padding:"4px 8px",fontSize:12,fontFamily:FT,outline:"none",width:120}}/>
                      <input value={weekendEditVal[`${day}-${i}`].activity||item.activity} onChange={e=>setWeekendEditVal(v=>({...v,[`${day}-${i}`]:{...(v[`${day}-${i}`]||{}),activity:e.target.value}}))}
                        style={{flex:1,border:"1.5px solid #c4b5fd",borderRadius:6,padding:"4px 8px",fontSize:12,fontFamily:FT,outline:"none",minWidth:180}}/>
                      <button onClick={()=>{
                    ...
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      ...
                      {isAdmin && <button onClick={()=>setWeekendEditVal(v=>({...v,[`${day}-${i}`]:{time:item.time,activity:item.activity}}))}
                        style={{background:"#f5f3ff",color:"#7c3aed",border:"none",borderRadius:8,padding:"5px 12px",cursor:"pointer",fontSize:11,fontWeight:"bold"}}>Edit</button>}
                    </div>
                  )}
                </div>
              </div>
      ...
      {tab==="saved" && (
        <div style={{display:"grid",gap:16}}>
          <div style={{background:"white",borderRadius:14,padding:20,boxShadow:"0 2px 12px rgba(0,0,0,.07)"}}>
            <div style={{fontWeight:"bold",color:"#3b0764",fontSize:15,marginBottom:12}}>💾 Saved Timetable Snapshots</div>
            ...
            {isAdmin && (
              <button onClick={()=>setShowSaveModal(true)}
                style={{background:"linear-gradient(135deg,#7c3aed,#3b0764)",color:"white",border:"none",borderRadius:9,padding:"9px 20px",cursor:"pointer",fontFamily:FT,fontSize:13,fontWeight:"bold",marginBottom:16}}>
              ...
              <div style={{display:"grid",gap:10}}>
                {savedTTs.map(snap=>(
                  <div key={snap.id} style={{background:"#f8fafc",borderRadius:12,padding:"14px 18px",border:"1px solid #e2e8f0",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
                    <div>
                      <div style={{fontWeight:"bold",color:"#3b0764",fontSize:14}}>📅 {snap.name}</div>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:9000,padding:16}}>
          <div style={{background:"white",borderRadius:18,padding:28,width:"95%",maxWidth:420,fontFamily:FT}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
              <div style={{fontWeight:"bold",color:"#3b0764",fontSize:16}}>💾 Save Timetable Snapshot</div>
...
function AttendancePage({students}) {
  ...
  const th={textAlign:"left",padding:"10px 12px",fontWeight:"bold",fontSize:11,color:"#7c3aed",background:"#f5f3ff"};
...
function DutyPage({staff,user,students,duties,setDuties,teacherAvail:availability,setTeacherAvail:setAvailability,stuRoster:stuDutyRoster,setStuRoster:setStuDutyRoster}) {
  ...
  function printRoster(type){
    ...
    <table style="width:100%;border-collapse:collapse;font-size:11px;">
      <thead><tr style="background:#3b0764;color:white;">
      ...
      <tbody>${rows.flatMap(({day,duties:ds})=>ds.map((d,i)=>`
        <tr style="background:${i%2===0?"rgba(255,255,255,0.5)":"rgba(248,250,252,0.5)"}">
  ...
  return (
    <div style={{padding:24}}>
        ...
        <div style={{display:"grid",gap:16}}>
          {user?.role==="admin"&&(
            <Card style={{background:"linear-gradient(135deg,#f5f3ff,#ede9fe)",border:"1px solid #ddd6fe"}}>
              <div style={{fontWeight:"bold",color:"#3b0764",marginBottom:8,fontSize:14}}>⚡ Auto-Generate Teacher Duty Roster</div>
                  ...
                  DAYS.flatMap(day=>{
                    ...
                    return dayDuties.map((d,i)=>(
                      <tr key={d.id} style={{background:i%2===0?"white":"#fafafa",borderTop:i===0?"2px solid #e2e8f0":"1px solid #f1f5f9"}}>
                        <td style={{...td,fontWeight:"bold",color:"#3b0764"}}>{i===0?day:""}</td>
                        <td style={td}><span style={{fontSize:10,padding:"2px 8px",borderRadius:20,fontWeight:"bold",background:d.session==="Evening"?"#fef3c7":"#f5f3ff",color:d.session==="Evening"?"#b45309":"#7c3aed"}}>{d.session||"Day"}</span></td>
      ...
      {tab==="availability"&&(
        <Card>
          <div style={{fontWeight:"bold",color:"#3b0764",marginBottom:4,fontSize:14}}>⚙️ Teacher Evening Duty Availability</div>
  ...
  return (
    <div style={{padding:24}}>
      <PageH title="📨 Messages Inbox" sub={`${unread} unread message${unread!==1?"s":""} · Auto-refreshes every 30s`}>
        <button onClick={loadMsgs} style={{background:"#f5f3ff",border:"1px solid #ddd6fe",borderRadius:8,padding:"6px 14px",cursor:"pointer",fontFamily:F,fontSize:12,color:"#7c3aed",fontWeight:"bold"}}>🔄 Refresh</button>
      </PageH>
      {loading ? (
        ...
        <Card style={{textAlign:"center",padding:40}}>
          ...
          <div style={{fontWeight:"bold",color:"#3b0764",fontSize:15}}>No messages yet</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1.4fr",gap:16,alignItems:"start"}}>
          <Card style={{padding:0}}>
            <div style={{padding:"12px 16px",background:"#3b0764",color:"white",fontWeight:"bold",fontSize:13,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            <div style={{maxHeight:520,overflowY:"auto"}}>
              {myMsgs.sort((a,b) => (b.id > a.id ? 1 : -1)).map(m => (
                <div key={m.id} onClick={() => { setSelected(m); markRead(m.id); setReplyText(""); setReplyMsg(""); }}
                  style={{padding:"12px 16px",borderBottom:"1px solid #f1f5f9",cursor:"pointer",background:selected?.id===m.id?"#f5f3ff":m.read?"white":"#fefce8",transition:"background .15s"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:3}}>
                    <div style={{fontWeight:"bold",fontSize:13,color:"#3b0764"}}>{m.fromName||m.from}</div>
                    {!m.read&&<span style={{background:"#7c3aed",color:"white",borderRadius:20,padding:"1px 7px",fontSize:10,fontWeight:"bold"}}>NEW</span>}
                  </div>
          ...
          {selected ? (
            <Card>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
                <div style={{fontWeight:"bold",color:"#3b0764",fontSize:14}}>Message from {selected.fromName||selected.from}</div>
              ...
              <div style={{display:"grid",gap:10,marginBottom:16}}>
                <div style={{background:"#f8fafc",borderRadius:10,padding:"10px 14px"}}>
                  ...
                  <div style={{fontSize:13,fontWeight:"bold",color:"#3b0764"}}>{selected.fromName||selected.from}</div>
                </div>
                <div style={{background:"#f8fafc",borderRadius:10,padding:"10px 14px"}}>
                ...
                <div style={{background:"#f5f3ff",borderRadius:10,padding:"14px 16px"}}>
                  ...
                  <div style={{fontSize:14,color:"#3b0764",lineHeight:1.6}}>{selected.message}</div>
                </div>
              </div>
              {selected.reply ? (
...
function SettingsPage({users,setUsers,logo,setLogo,ttSetup,setTtSetup}) {
  ...
  const [editAllocCls,setEditAllocCls]=useState("Grade 7");
  const [editAllocSub,setEditAllocSub]=useState("");
  function openEdit(u){
    setEditForm({...u});
    setEditUser(u);
    setEditAllocCls("Grade 7");
    setEditAllocSub("");
  }
  function saveEdit(){
    if(!editForm.name||!editForm.username) return;
    // Find the old name before saving
    const oldName=(editUser?.name)||"";
    const newName=editForm.name;
    setUsers(p=>p.map(u=>u.id===editForm.id?{...editForm}:u));
    // If name changed, update all subjectTeachers entries that reference the old name
    if(oldName && newName && oldName!==newName && setTtSetup){
      setTtSetup(prev=>{
        const prevST={...(prev?.setupData?.subjectTeachers||{})};
        ...
        Object.keys(updated).forEach(key=>{
          ...
          if(idx!==-1){
            arr[idx]=newName;
            // Deduplicate in case new name was already in the array
            updated[key]=[...new Set(arr.filter(Boolean))];
          }
        });
        return {...prev,setupData:{...(prev?.setupData||{}),subjectTeachers:updated}};
      });
    }
    setEditUser(null);
  }
  function addEditAlloc(){
    const sub=editAllocSub||getSubs(editAllocCls)[0];
    if(!sub||!setTtSetup) return;
    setTtSetup(prev=>{
      const prevST=(prev?.setupData?.subjectTeachers)||{};
      const updated={...prevST};
      const parts=COMBINED_SUBJECT_PARTS[sub]||[sub];
      parts.forEach(part=>{
        const key=`${editAllocCls}::${part}`;
        const existing=updated[key];
        const arr=Array.isArray(existing)?[...existing]:(existing?[existing]:[]);
        if(!arr.includes(editForm.name)) arr.push(editForm.name);
        updated[key]=arr;
      });
      return {...prev,setupData:{...(prev?.setupData||{}),subjectTeachers:updated}};
    });
  ...
  function removeEditAlloc(key,name){
    if(!setTtSetup) return;
    setTtSetup(prev=>{
      const prevST={...(prev?.setupData?.subjectTeachers||{})};
      const arr=getTeachersForKey(prevST,key).filter(n=>n!==name);
      if(arr.length===0) delete prevST[key];
      else prevST[key]=arr;
      return {...prev,setupData:{...(prev?.setupData||{}),subjectTeachers:prevST}};
    });
  }
  // Subject allocations for new account
  const [allocCls,setAllocCls]=useState("Grade 7");
  const [allocSub,setAllocSub]=useState("");
  const [allocList,setAllocList]=useState([]); // [{cls,subject}]
  function addAlloc(){
    const sub=allocSub||getSubs(allocCls)[0];
    if(!sub) return;
    if(allocList.find(a=>a.cls===allocCls&&a.subject===sub)) return;
    setAllocList(p=>[...p,{cls:allocCls,subject:sub}]);
    setAllocSub("");
  }
  function removeAlloc(cls,subject){setAllocList(p=>p.filter(a=>!(a.cls===cls&&a.subject===subject)));}
  ...
  const flash=(t,ok=true)=>{setMsg({t,ok});setTimeout(()=>setMsg({t:"",ok:true}),2500);};
  function doAdd(){
    if(!form.name||!form.username||!form.password) return flash("Name, username and password required.",false);
    if(users.find(u=>u.username===form.username)) return flash("Username already exists.",false);
    setUsers(p=>[...p,{...form,id:Date.now().toString()}]);
    // Write class+subject allocations into timetable subjectTeachers map (append, don't overwrite)
    if(allocList.length>0 && setTtSetup){
      setTtSetup(prev=>{
        const prevST=(prev?.setupData?.subjectTeachers)||{};
        const updated={...prevST};
        allocList.forEach(({cls,subject})=>{
          const parts=COMBINED_SUBJECT_PARTS[subject]||[subject];
          parts.forEach(part=>{
            const key=`${cls}::${part}`;
            const existing=updated[key];
            const arr=Array.isArray(existing)?existing:(existing?[existing]:[]);
            if(!arr.includes(form.name)) arr.push(form.name);
            updated[key]=arr;
          });
        });
        return {...prev,setupData:{...(prev?.setupData||{}),subjectTeachers:updated}};
  ...
  return (
    <div style={{padding:24,fontFamily:appFont}}>
      ...
      {/* EDIT USER MODAL */}
      {editUser&&<Modal title={`✏️ Edit Account — ${editUser.name}`} onClose={()=>setEditUser(null)}>
        <div style={{display:"grid",gap:10,maxHeight:"70vh",overflowY:"auto",paddingRight:4}}>
          <Inp label="FULL NAME *" value={editForm.name} onChange={v=>setEditForm(p=>({...p,name:v}))} placeholder="Ms. Purity"/>
          <Inp label="USERNAME *" value={editForm.username} onChange={v=>setEditForm(p=>({...p,username:v}))} placeholder="purity"/>
          <Inp label="EMAIL" value={editForm.email||""} onChange={v=>setEditForm(p=>({...p,email:v}))} placeholder="purity@tnks.sc.ke" type="email"/>
          <Inp label="PHONE" value={editForm.phone||""} onChange={v=>setEditForm(p=>({...p,phone:v}))} placeholder="+254 7..."/>
          <Sel label="ROLE" value={editForm.role||"teacher"} onChange={v=>setEditForm(p=>({...p,role:v}))} options={["teacher","admin"]}/>
          <Sel label="STAFF TYPE" value={editForm.staffType||"teaching"} onChange={v=>setEditForm(p=>({...p,staffType:v}))} options={["teaching","non-teaching"]}/>
          <Sel label="CONTACT ROLE" value={editForm.contactRole||"teacher"} onChange={v=>setEditForm(p=>({...p,contactRole:v}))} options={["teacher","dean","secretary","director","manager","admin","non_teaching"]}/>
          <Inp label="SUBJECT/DEPT (display)" value={editForm.subject||""} onChange={v=>setEditForm(p=>({...p,subject:v}))} placeholder="e.g. Mathematics"/>
          {/* Subject allocation manager */}
          {editForm.staffType==="teaching"&&<div style={{borderTop:"1px solid #e2e8f0",paddingTop:10}}>
            <div style={{fontSize:11,fontWeight:"bold",color:"#3b0764",marginBottom:8}}>📚 ASSIGNED CLASSES & SUBJECTS</div>
            {/* Current allocations with remove buttons */}
            {(()=>{
              const sm=ttSetup?.setupData?.subjectTeachers||{};
              const assigned=Object.keys(sm).filter(k=>getTeachersForKey(sm,k).includes(editForm.name));
              return assigned.length>0?(
                <div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:10}}>
                  {assigned.map(key=>{
                    const [cls,...rest]=key.split("::");const sub=rest.join("::");
                    return(
                      <span key={key} style={{background:"#ede9fe",color:"#3b0764",fontSize:10,padding:"3px 8px",borderRadius:20,fontWeight:"bold",display:"flex",alignItems:"center",gap:4}}>
                        {cls} — {SUBJECT_SHORT[sub]||sub}
                        <button onClick={()=>removeEditAlloc(key,editForm.name)} style={{background:"none",border:"none",color:"#b91c1c",cursor:"pointer",fontSize:13,lineHeight:1,padding:0,fontWeight:"bold"}}>×</button>
                      </span>
                    );
                  })}
                </div>
              ):<div style={{fontSize:10,color:"#94a3b8",fontStyle:"italic",marginBottom:8}}>No subjects assigned yet.</div>;
            })()}
            {/* Add new allocation */}
            <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"flex-end"}}>
              <div style={{flex:1,minWidth:90}}>
                <label style={{fontSize:10,fontWeight:"bold",color:"#374151",display:"block",marginBottom:3}}>CLASS</label>
                <select value={editAllocCls} onChange={e=>{setEditAllocCls(e.target.value);setEditAllocSub("");}} style={{width:"100%",border:"1.5px solid #e2e8f0",borderRadius:7,padding:"6px 8px",fontSize:12,fontFamily:F}}>
                  {ALL_CLASSES.map(c=><option key={c} value={c}>{c}</option>)}
                </select>
              </div>
              <div style={{flex:2,minWidth:120}}>
                <label style={{fontSize:10,fontWeight:"bold",color:"#374151",display:"block",marginBottom:3}}>SUBJECT</label>
                <select value={editAllocSub} onChange={e=>setEditAllocSub(e.target.value)} style={{width:"100%",border:"1.5px solid #e2e8f0",borderRadius:7,padding:"6px 8px",fontSize:12,fontFamily:F}}>
                  <option value="">-- pick subject --</option>
                  {getSubs(editAllocCls).map(s=><option key={s} value={s}>{s}</option>)}
                </select>
              </div>
              <button onClick={addEditAlloc} style={{background:"#7c3aed",color:"white",border:"none",borderRadius:7,padding:"7px 12px",fontSize:12,cursor:"pointer",fontWeight:"bold",alignSelf:"flex-end"}}>+ Add</button>
            </div>
          </div>}
        </div>
        <div style={{display:"flex",gap:8,marginTop:16}}><Btn onClick={saveEdit} v="primary">💾 Save Changes</Btn><Btn onClick={()=>setEditUser(null)} v="ghost">Cancel</Btn></div>
      </Modal>}
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18,marginBottom:18}}>
        ...
        <Card style={{borderLeft:"4px solid #7c3aed"}}>
          <div style={{fontWeight:"bold",color:"#3b0764",marginBottom:14,fontSize:14}}>🖼️ School Logo</div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1.5fr",gap:18,alignItems:"start"}}>
        <Card>
          ...
          <div style={{display:"grid",gap:10}}>
            ...
            <Inp label="SUBJECT/DEPT (display)" value={form.subject} onChange={v=>setForm({...form,subject:v})} placeholder="e.g. Mathematics"/>
            {/* ── Class + Subject allocation ── */}
            {form.staffType==="teaching"&&<div style={{borderTop:"1px solid #e2e8f0",paddingTop:10}}>
              <div style={{fontSize:11,fontWeight:"bold",color:"#3b0764",marginBottom:8}}>📚 ASSIGN CLASSES & SUBJECTS</div>
              <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"flex-end",marginBottom:8}}>
                <div style={{flex:1,minWidth:90}}>
                  <label style={{fontSize:10,fontWeight:"bold",color:"#374151",display:"block",marginBottom:3}}>CLASS</label>
                  <select value={allocCls} onChange={e=>{setAllocCls(e.target.value);setAllocSub("");}} style={{width:"100%",border:"1.5px solid #e2e8f0",borderRadius:7,padding:"6px 8px",fontSize:12,fontFamily:F}}>
                    {ALL_CLASSES.map(c=><option key={c} value={c}>{c}</option>)}
                  </select>
                </div>
                <div style={{flex:2,minWidth:120}}>
                  <label style={{fontSize:10,fontWeight:"bold",color:"#374151",display:"block",marginBottom:3}}>SUBJECT</label>
                  <select value={allocSub} onChange={e=>setAllocSub(e.target.value)} style={{width:"100%",border:"1.5px solid #e2e8f0",borderRadius:7,padding:"6px 8px",fontSize:12,fontFamily:F}}>
                    <option value="">-- pick subject --</option>
                    {getSubs(allocCls).map(s=><option key={s} value={s}>{s}</option>)}
                  </select>
                </div>
                <button onClick={addAlloc} style={{background:"#7c3aed",color:"white",border:"none",borderRadius:7,padding:"7px 12px",fontSize:12,cursor:"pointer",fontWeight:"bold",alignSelf:"flex-end"}}>+ Add</button>
              </div>
              {allocList.length>0&&<div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                {allocList.map((a,i)=>(
                  <span key={i} style={{background:"#ede9fe",color:"#3b0764",fontSize:10,padding:"3px 8px",borderRadius:20,fontWeight:"bold",display:"flex",alignItems:"center",gap:4}}>
                    {a.cls} — {a.subject}
                    <button onClick={()=>removeAlloc(a.cls,a.subject)} style={{background:"none",border:"none",color:"#b91c1c",cursor:"pointer",fontSize:12,lineHeight:1,padding:0}}>×</button>
                  </span>
                ))}
              </div>}
              {allocList.length===0&&<div style={{fontSize:10,color:"#94a3b8",fontStyle:"italic"}}>No subjects assigned yet. Add at least one above.</div>}
            </div>}
          </div>
          {msg.t&&<div style={{marginTop:10,fontSize:13,color:msg.ok?"#1d4ed8":"#b91c1c",fontWeight:"bold"}}>{msg.t}</div>}
        ...
        <Card style={{padding:0}}>
          <div style={{padding:"12px 16px",background:"#f5f3ff",fontWeight:"bold",color:"#3b0764",fontSize:13,borderBottom:"1px solid #ede9fe"}}>Staff Accounts ({users.length})</div>
          <div style={{overflowX:"auto",WebkitOverflowScrolling:"touch"}}><table style={{width:"100%",borderCollapse:"collapse",minWidth:600}}>
            <thead><tr>{["Name","Username","Password","Role","Contact As","Subjects","Actions"].map(h=><th key={h} style={th}>{h}</th>)}</tr></thead>
            <tbody>{users.map((u,i)=><tr key={u.id} style={{background:i%2===0?"white":"#fafafa"}}>
              ...
              <td style={{...td,fontSize:10,maxWidth:160}}>
                {(()=>{
                  const sm=ttSetup?.setupData?.subjectTeachers||PRELOADED_TEACHERS;
                  const assigned=Object.keys(sm).filter(k=>getTeachersForKey(sm,k).includes(u.name));
                  if(!assigned.length) return <span style={{color:"#94a3b8"}}>—</span>;
                  // Group by class
                  const byClass={};
                  assigned.forEach(k=>{const[cls,...rest]=k.split("::");const sub=rest.join("::");if(!byClass[cls])byClass[cls]=[];byClass[cls].push(sub);});
                  return Object.entries(byClass).map(([cls,subs])=>(
                    <div key={cls} style={{marginBottom:2}}>
                      <span style={{fontWeight:"bold",color:"#3b0764"}}>{cls}:</span>{" "}
                      <span style={{color:"#64748b"}}>{subs.map(s=>SUBJECT_SHORT[s]||s.split(" ")[0]).join(", ")}</span>
                    </div>
                  ));
                })()}
              </td>
              <td style={td}>
                <button onClick={()=>openEdit(u)} style={{color:"#166534",background:"none",border:"none",cursor:"pointer",fontSize:11,marginRight:8,fontWeight:"bold"}}>✏️ Edit</button>
    ...
    <div style={{ marginTop: 20 }}>
      ...
      {panel === "menu" && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <button onClick={() => setPanel("ai")} style={{ background: "linear-gradient(135deg,#f5f3ff,#ede9fe)", border: "1.5px solid #ddd6fe", borderRadius: 14, padding: "18px 16px", cursor: "pointer", fontFamily: F, textAlign: "left" }}>
            <div style={{ fontSize: 32, marginBottom: 8 }}>🤖</div>
          ...
          <div ref={chatRef} style={{ height: 320, overflowY: "auto", padding: 14, display: "flex", flexDirection: "column", gap: 10 }}>
            {chatHistory.map((m, i) => (
              <div key={i} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start" }}>
                <div style={{ maxWidth: "85%", background: m.role === "user" ? "#3b0764" : "#f1f5f9", color: m.role === "user" ? "white" : "#374151", borderRadius: 12, padding: "10px 14px", fontSize: 13, lineHeight: 1.5 }}>{m.content}</div>
              </div>
            ))}
          ...
          <div style={{ padding: "8px 14px", borderTop: "1px solid #f1f5f9", background: "#fafafa" }}>
            ...
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", maxHeight: 80, overflowY: "auto" }}>
              {PARENT_SUGGESTED_QUESTIONS.map(q => (
                <button key={q} onClick={() => setChatMsg(q)} style={{ background: "#f5f3ff", border: "1px solid #ddd6fe", borderRadius: 20, padding: "4px 10px", cursor: "pointer", fontFamily: F, fontSize: 10, color: "#7c3aed", whiteSpace: "nowrap" }}>{q}</button>
              ))}
            </div>
          </div>
        ...
        <div style={{ background: "white", borderRadius: 16, boxShadow: "0 4px 20px rgba(0,0,0,.08)", overflow: "hidden" }}>
          ...
          <div style={{ padding: 18 }}>
            <div style={{ background: "#f8fafc", borderRadius: 12, padding: "12px 16px", display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
              ...
              <div>
                <div style={{ fontWeight: "bold", color: "#3b0764", fontSize: 13 }}>To: {selectedStaff.name}</div>
  ...
  return (
    <div style={{padding:24}}>
      <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:22,background:"white",borderRadius:14,padding:18,boxShadow:"0 2px 10px rgba(0,0,0,.07)"}}>
        ...
        <div><div style={{fontSize:18,fontWeight:"bold",color:"#3b0764"}}>{student.name}</div><div style={{fontSize:12,color:"#64748b"}}>{student.class} · Adm: {student.admNo}</div><div style={{fontSize:12,color:"#64748b"}}>{student.parentName&&`Parent: ${student.parentName}`}</div></div>
      ...
      {tab==="notices"&&<div style={{display:"grid",gap:12}}>{recentAnn.length?recentAnn.map(a=><Card key={a.id} style={{borderLeft:"4px solid #7c3aed",padding:"12px 16px"}}><div style={{fontWeight:"bold",color:"#3b0764",marginBottom:4}}>{a.title}</div><div style={{fontSize:12,color:"#374151"}}>{a.body}</div><div style={{fontSize:11,color:"#94a3b8",marginTop:6}}>{a.date}</div></Card>):<Empty icon="📌" text="No notices."/>}</div>}
...
function LearnerMonitoringPage({students,user,monitoring,setMonitoring}) {
  ...
  return (
    <div style={{padding:24}}>
      <div style={{fontWeight:"bold",color:"#3b0764",fontSize:22,fontFamily:F,marginBottom:4}}>🏥 Learner Monitoring</div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:10,marginBottom:18}}>
        ...
        <Stat icon="📋" label="Total Records" value={records.length} color="#7c3aed"/>
      </div>
...
function FeeStructurePage({user,logo,feeStructure,setFeeStructure}){
  ...
  function printFeeStructure(){
    ...
    printWindow("Fee Structure",`<h3 style="color:#3b0764;">Fee Structure — ${new Date().getFullYear()}</h3>
      <table style="width:100%;border-collapse:collapse;font-size:12px;">
        <thead><tr style="background:#3b0764;color:white;">${["Student Type","Term 1","Term 2","Term 3","Annual Total"].map(h=>`<th style="padding:10px 12px;text-align:left;">${h}</th>`).join("")}</tr></thead>
  ...
  return (
    <div style={{padding:24}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20,flexWrap:"wrap",gap:10}}>
        <div><h2 style={{margin:0,color:"#3b0764",fontSize:22,fontFamily:F}}>📑 Fee Structure</h2><div style={{fontSize:13,color:"#64748b",marginTop:2}}>Fees payable per term by student type</div></div>
        <div style={{display:"flex",gap:8}}>
      ...
      <div style={{display:"grid",gap:16}}>
        {STUDENT_TYPES.map(type=>(
          <Card key={type}>
            <div style={{fontWeight:"bold",color:"#3b0764",fontSize:15,marginBottom:14,display:"flex",alignItems:"center",gap:8}}>
                ...
                <div key={t} style={{background:"#f8fafc",borderRadius:10,padding:"14px 16px",textAlign:"center",border:"1.5px solid #e2e8f0"}}>
                  ...
                  {editMode?(
                    <input type="number" value={draft[type]?.[t]||0}
                      ...
                      style={{width:"100%",border:"1.5px solid #c4b5fd",borderRadius:8,padding:"8px",fontSize:16,fontWeight:"bold",textAlign:"center",fontFamily:"Georgia,serif",color:"#3b0764",boxSizing:"border-box"}}/>
                  ):(
                    <div style={{fontSize:20,fontWeight:"bold",color:"#3b0764"}}>KES {(displayStructure[type]?.[t]||0).toLocaleString()}</div>
                  )}
  ...
  return(
    <div style={{padding:24}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:16}}>
        {/* School Details */}
        <Card>
          <div style={{fontWeight:"bold",color:"#3b0764",fontSize:15,marginBottom:14}}>🏫 School Details</div>
          <div style={{display:"grid",gap:10}}>
            {infoRows.map(row=>(
              <div key={row.label} style={{display:"flex",gap:12,alignItems:"flex-start",padding:"8px 12px",background:"#f8fafc",borderRadius:8}}>
                ...
                <div>
                  ...
                  <div style={{fontSize:13,color:"#3b0764",fontWeight:"bold"}}>{row.value}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{display:"grid",gap:16,alignContent:"start"}}>
          ...
          <Card>
            <div style={{fontWeight:"bold",color:"#3b0764",fontSize:15,marginBottom:14}}>📞 Contact Us</div>
            ...
            <div style={{marginTop:14,padding:"10px 14px",background:"#f8fafc",borderRadius:8,fontSize:12}}>
              ...
              <div style={{color:"#7c3aed",fontWeight:"bold"}}>{SCHOOL.phone}</div>
          ...
          <Card style={{background:"#f8fafc",textAlign:"center",padding:"20px 16px"}}>
            <div style={{fontSize:36,marginBottom:8}}>📍</div>
            <div style={{fontWeight:"bold",color:"#3b0764",fontSize:13}}>{SCHOOL.address}</div>
            <div style={{color:"#64748b",fontSize:11,marginTop:4}}>{SCHOOL.location}</div>
            <button onClick={()=>window.open("https://maps.app.goo.gl/rGYT8sXkSSrNKWNW8")} style={{marginTop:12,background:"#3b0764",color:"white",border:"none",borderRadius:9,padding:"8px 20px",cursor:"pointer",fontFamily:F,fontSize:12,fontWeight:"bold"}}>
              📍 Open in Maps
            </button>
          </Card>
        </div>
...
function ExamManagementPage({ students, staff, user, examSchedules, setExamSchedules, logo }) {
  ...
  function printExamTimetable() {
    ...
    const html = `<h3 style="margin:0 0 14px;color:#3b0764;">Examination Timetable — ${form.term} ${form.year}</h3>
      <table style="width:100%;border-collapse:collapse;font-size:11px;">
        <thead><tr style="background:#3b0764;color:white;">
  ...
  function printInvigilationSchedule() {
    ...
    const html = Object.entries(byInv).map(([inv, exams]) => `
      <div style="page-break-after:always;padding:16px;">
        ${buildSectionHeader(logo)}
        <h3 style="color:#3b0764;margin:0 0 10px;">Invigilation — ${inv}</h3>
        <table style="width:100%;border-collapse:collapse;font-size:11px;">
          <thead><tr style="background:#3b0764;color:white;">
    ...
    <div style={{ padding: 24 }}>
      ...
      {tab === "add" && <Card style={{ marginBottom: 18 }}>
        ...
        <div style={{ marginTop: 14, paddingTop: 12, borderTop: "1px dashed #e2e8f0" }}>
          <div style={{ fontSize: 12, fontWeight: "bold", color: "#3b0764", marginBottom: 10 }}>👤 Assign Invigilators</div>
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 700 }}>
            ...
            <tbody>
              {(tab === "schedule" ? upcoming : past).length ? (tab === "schedule" ? upcoming : past).map((e, i) => <tr key={e.id} style={{ background: i % 2 === 0 ? "white" : "#fafafa" }}>
                ...
                <td style={td}><span style={{ background: "#f5f3ff", color: "#7c3aed", fontSize: 10, padding: "2px 8px", borderRadius: 20, fontWeight: "bold" }}>{e.class}</span></td>
      ...
      {tab === "invigilation" && (() => {
        ...
        return <div style={{ display: "grid", gap: 14 }}>
          {Object.keys(byInv).length ? Object.entries(byInv).map(([inv, exams]) => <Card key={inv}>
            <div style={{ fontWeight: "bold", color: "#3b0764", fontSize: 14, marginBottom: 10 }}>👤 {inv} — {exams.length} exam(s)</div>
            <div style={{ display: "grid", gap: 8 }}>
              {exams.sort((a, b) => new Date(a.date) - new Date(b.date)).map(e => <div key={e.id} style={{ display: "flex", gap: 12, alignItems: "center", padding: "8px 12px", background: "#f8fafc", borderRadius: 8 }}>
                ...
                <span style={{ background: "#f5f3ff", color: "#7c3aed", fontSize: 10, padding: "2px 8px", borderRadius: 20, fontWeight: "bold" }}>{e.class}</span>
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      {tab === "students" && <>
        ...
        <Card style={{ marginBottom: 14 }}>
          <div style={{ fontWeight: "bold", color: "#3b0764", marginBottom: 12, fontSize: 14 }}>📁 Upload CSV File</div>
          <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
        ...
        {preview.length > 0 && <Card style={{ padding: 0, overflow: "hidden" }}>
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              <thead><tr>{Object.keys(preview[0] || {}).map(k => <th key={k} style={{ padding: "8px 12px", background: "#f5f3ff", textAlign: "left", fontSize: 11, fontWeight: "bold", color: "#7c3aed" }}>{k}</th>)}</tr></thead>
      ...
      {tab === "results" && <Card>
        <div style={{ fontWeight: "bold", color: "#3b0764", marginBottom: 14, fontSize: 14 }}>📝 Bulk Results Entry</div>
        <div style={{ fontSize: 12, color: "#64748b", marginBottom: 14, background: "#f5f3ff", padding: "10px 14px", borderRadius: 8 }}>
...
function BulkFeesTab({ students, fees, setFees }) {
  ...
  return <Card>
    <div style={{ fontWeight: "bold", color: "#3b0764", marginBottom: 14, fontSize: 14 }}>💰 Bulk Fee Recording</div>
    ...
    <div style={{ padding: 24 }}>
      ...
      {tab === "compose" && <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, alignItems: "start" }}>
        <div style={{ display: "grid", gap: 14 }}>
          <Card>
            <div style={{ fontWeight: "bold", color: "#3b0764", marginBottom: 14, fontSize: 14 }}>🎯 Recipients</div>
            <div style={{ display: "grid", gap: 10 }}>
              <div>
          ...
          <Card>
            <div style={{ fontWeight: "bold", color: "#3b0764", marginBottom: 10, fontSize: 14 }}>📋 Quick Templates</div>
            <div style={{ display: "grid", gap: 6 }}>
              {[["fee_reminder", "💰 Fee Reminder"], ["absent_alert", "🏠 Absence Alert"], ["results_ready", "📝 Results Ready"], ["general", "✍️ Custom Message"]].map(([k, l]) =>
                <button key={k} onClick={() => applyTemplate(k)} style={{ background: template === k ? "#3b0764" : "#f8fafc", color: template === k ? "white" : "#374151", border: "1px solid #e2e8f0", borderRadius: 8, padding: "8px 14px", cursor: "pointer", fontSize: 12, fontFamily: "Georgia,serif", textAlign: "left", fontWeight: template === k ? "bold" : "normal" }}>{l}</button>
              )}
            </div>
          </Card>
        </div>
        <Card>
          <div style={{ fontWeight: "bold", color: "#3b0764", marginBottom: 14, fontSize: 14 }}>✉️ Message</div>
          <div style={{ display: "grid", gap: 12 }}>
      ...
      {tab === "log" && <Card style={{ padding: 0, overflow: "hidden" }}>
        <div style={{ padding: "12px 16px", background: "#f5f3ff", fontWeight: "bold", color: "#3b0764", fontSize: 13, borderBottom: "1px solid #ede9fe" }}>Notification Log ({log.length})</div>
        {log.length ? <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead><tr>{["Time", "Channel", "Recipients", "Subject/Type", "Sent By", "Status"].map(h => <th key={h} style={{ textAlign: "left", padding: "9px 12px", fontWeight: "bold", fontSize: 11, color: "#7c3aed", background: "#f5f3ff" }}>{h}</th>)}</tr></thead>
          <tbody>{log.map((l, i) => <tr key={l.id} style={{ background: i % 2 === 0 ? "white" : "#fafafa", borderTop: "1px solid #f1f5f9" }}>
            ...
            <td style={{ padding: "8px 12px", fontSize: 12 }}><span style={{ background: "#f5f3ff", color: "#7c3aed", fontSize: 10, padding: "2px 8px", borderRadius: 20, fontWeight: "bold" }}>{l.channel}</span></td>
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1.4fr", gap: 18, alignItems: "start" }}>
        <div style={{ display: "grid", gap: 14 }}>
          <Card>
            <div style={{ fontWeight: "bold", color: "#3b0764", marginBottom: 14, fontSize: 14 }}>Select Student</div>
            <div style={{ display: "grid", gap: 10 }}>
          ...
          {student && (() => {
            ...
            return <Card style={{ background: "#f8fafc" }}>
              <div style={{ fontWeight: "bold", color: "#3b0764", marginBottom: 10, fontSize: 13 }}>Performance Summary</div>
        ...
        <div style={{ display: "grid", gap: 14 }}>
          <Card>
            <div style={{ fontWeight: "bold", color: "#3b0764", marginBottom: 14, fontSize: 14 }}>AI-Generated Comment</div>
    ...
    <div style={{ padding: 24 }}>
      <PageH title="📅 School Calendar" sub="Visual monthly calendar with events and activities">
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <span style={{ fontWeight: "bold", color: "#3b0764", fontSize: 15, minWidth: 160, textAlign: "center" }}>{MONTHS[calMonth]} {calYear}</span>
      ...
      {selected && (() => {
        ...
        return <Modal title={`Events — ${selected} ${MONTHS[calMonth]} ${calYear}`} onClose={() => setSelected(null)}>
          {dayEvs.length ? dayEvs.map(e => <div key={e.id} style={{ background: "#f8fafc", borderRadius: 8, padding: "10px 14px", marginBottom: 10, borderLeft: `4px solid ${TYPE_COLORS[e.type] || "#64748b"}` }}>
            <div style={{ fontWeight: "bold", color: "#3b0764" }}>{e.title}</div>
      ...
      <Card style={{ padding: 0, overflow: "hidden" }}>
        {/* Day headers */}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", background: "#3b0764" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)" }}>
          {days.map((day, idx) => {
            ...
            return <div key={idx} onClick={() => { if (day) setSelected(day); }} style={{ minHeight: 90, padding: "6px 8px", borderRight: "1px solid #f1f5f9", borderBottom: "1px solid #f1f5f9", background: isToday ? "#f5f3ff" : day ? "white" : "#fafafa", cursor: day ? "pointer" : "default", transition: "background .1s" }} onMouseEnter={e => { if (day) e.currentTarget.style.background = "#f8fafc"; }} onMouseLeave={e => { e.currentTarget.style.background = isToday ? "#f5f3ff" : day ? "white" : "#fafafa"; }}>
              {day && <>
                <div style={{ fontSize: 13, fontWeight: isToday ? "bold" : "normal", color: isToday ? "#7c3aed" : "#374151", width: 24, height: 24, borderRadius: "50%", background: isToday ? "#ede9fe" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 4 }}>{day}</div>
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: selClub ? "1fr 1.2fr" : "1fr", gap: 16 }}>
            ...
            {(clubs || []).length ? (clubs || []).map(club => {
              ...
              return <div key={club.id} onClick={() => setSelClub(selClub === club.id ? null : club.id)} style={{ background: selClub === club.id ? "#f5f3ff" : "white", borderRadius: 12, padding: "14px 18px", boxShadow: "0 1px 6px rgba(0,0,0,.06)", border: `1px solid ${selClub === club.id ? "#c4b5fd" : "#e2e8f0"}`, cursor: "pointer", borderLeft: `4px solid ${catColor}` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div>
                    <div style={{ fontWeight: "bold", color: "#3b0764", fontSize: 14 }}>{club.name}</div>
                    <div style={{ fontSize: 12, color: "#64748b", marginTop: 3 }}>
        ...
        {sel && <Card>
          <div style={{ fontWeight: "bold", color: "#3b0764", fontSize: 15, marginBottom: 14 }}>👥 {sel.name} — Members</div>
...
// ══════════════════════════════════════════════════════════
function TransportPage({ students, setStudents, user, transportRoutes: _transportRoutes, setTransportRoutes, busMonitoring, setBusMonitoring }) {
  const PALETTE = ["#7c3aed","#1d4ed8","#b45309","#166534","#be185d","#0891b2"];
  const DEFAULT_ROUTES = {
    "Route A":{driver:"",vehicle:"",capacity:40,fare:15000,color:"#7c3aed",destinations:["Meru Town","Mukothima Junction","School Gate"],timeDeparted:"",timeArrived:""},
    "Route B":{driver:"",vehicle:"",capacity:40,fare:16500,color:"#1d4ed8",destinations:["Gatunga","Igamba","School Gate"],timeDeparted:"",timeArrived:""},
    "Route C":{driver:"",vehicle:"",capacity:40,fare:18000,color:"#b45309",destinations:["Tharaka","Nkondi","School Gate"],timeDeparted:"",timeArrived:""},
  };
  // Always ensure we have a valid object; migrate old keys that lack color/fare
  const rawRoutes = (_transportRoutes && typeof _transportRoutes === "object" && Object.keys(_transportRoutes).length > 0)
    ? _transportRoutes : DEFAULT_ROUTES;
  const transportRoutes = Object.fromEntries(
    Object.entries(rawRoutes).map(([k,v],i) => [k, {
      driver:"", vehicle:"", capacity:40, fare:15000,
      color: PALETTE[i % PALETTE.length],
      destinations:[], timeDeparted:"", timeArrived:"",
      ...v,
  ...
  const ROUTES = Object.keys(transportRoutes);
  // For backwards compat with old busRoute values
  const routeColor = (r) => transportRoutes[r]?.color || PALETTE[ROUTES.indexOf(r) % PALETTE.length] || "#7c3aed";
  const routeFare  = (r) => transportRoutes[r]?.fare || 15000;
  // Add / rename route helpers
  const [newRouteName, setNewRouteName] = React.useState("");
  const [renameMap, setRenameMap] = React.useState({}); // { oldName: newNameBeingTyped }
  function addRoute() {
    const name = newRouteName.trim();
    if (!name || transportRoutes[name]) return flash("Enter a unique route name.", false);
    const color = PALETTE[ROUTES.length % PALETTE.length];
    setTransportRoutes(p => ({ ...p, [name]: { driver:"", vehicle:"", capacity:40, fare:15000, color, destinations:[], timeDeparted:"", timeArrived:"" } }));
    setNewRouteName(""); flash("✅ Route added!");
  }
  function deleteRoute(route) {
    if (busStudents(route).length > 0) return flash(`Remove all students from ${route} before deleting.`, false);
    setTransportRoutes(p => { const n = {...p}; delete n[route]; return n; });
    flash(`🗑️ ${route} deleted.`);
  }
  function renameRoute(oldName) {
    const newName = (renameMap[oldName]||"").trim();
    if (!newName || newName === oldName) return setRenameMap(p=>({...p,[oldName]:undefined}));
    if (transportRoutes[newName]) return flash("A route with that name already exists.", false);
    // Update transportRoutes key
    setTransportRoutes(p => {
      const n = {};
      for (const [k,v] of Object.entries(p)) { n[k===oldName?newName:k] = v; }
      return n;
    });
    // Update students on this route
    setStudents(p => p.map(s => s.busRoute===oldName ? {...s, busRoute:newName, studentType:`Bus (${newName})`} : s));
    setRenameMap(p=>({...p,[oldName]:undefined}));
  ...
  function assignStudent() {
    ...
    setStudents(p => p.map(s => s.id === assignId ? { ...s, busRoute: selRoute } : s));
    setAssignId(""); flash(`✅ ${stu.name} assigned to ${selRoute}!`);
  }

  function removeFromBus(id) {
    setStudents(p => p.map(s => s.id === id ? { ...s, busRoute: "" } : s));
  }
  ...
  function deleteMonLog(id) {
    setBusMonitoring(p => p.filter(x => x.id !== id));
  }

  const busStudents = (route) => students.filter(s => s.busRoute === route);
  const unassigned = students.filter(s => !s.busRoute && (s.studentType||"Day Scholar") === "Day Scholar");
    ...
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(170px,1fr))", gap: 12, marginBottom: 18 }}>
        {ROUTES.map(r => {
          ...
          return <Stat key={r} icon="🚌" label={r} value={`${count}/${cap}`} color={routeColor(r)} sub={`KES ${routeFare(r).toLocaleString()}/term`} />;
        })}
      ...
      {tab === "routes" && <div style={{ display: "grid", gap: 14 }}>
        {ROUTES.map(route => {
          ...
          return <Card key={route} style={{ borderLeft: `4px solid ${routeColor(route)}` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
              <div style={{ flex: 1, minWidth: 260 }}>
                <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:6 }}>
                  {renameMap[route]!==undefined
                    ? <><input autoFocus value={renameMap[route]||""} onChange={e=>setRenameMap(p=>({...p,[route]:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&renameRoute(route)} style={{fontWeight:"bold",fontSize:14,border:"1.5px solid #c4b5fd",borderRadius:6,padding:"3px 8px",fontFamily:"Georgia,serif",width:160}}/><button onClick={()=>renameRoute(route)} style={{background:"#7c3aed",color:"white",border:"none",borderRadius:6,padding:"3px 10px",cursor:"pointer",fontSize:12,fontFamily:"Georgia,serif"}}>✓</button><button onClick={()=>setRenameMap(p=>({...p,[route]:undefined}))} style={{background:"#f1f5f9",color:"#64748b",border:"none",borderRadius:6,padding:"3px 8px",cursor:"pointer",fontSize:12,fontFamily:"Georgia,serif"}}>✕</button></>
                    : <><span style={{fontWeight:"bold",color:"#3b0764",fontSize:15}}>{route}</span>{user.role==="admin"&&<button onClick={()=>setRenameMap(p=>({...p,[route]:route}))} style={{background:"none",border:"none",cursor:"pointer",color:"#94a3b8",fontSize:11,padding:0}} title="Rename route">✏️</button>}</>}
                </div>
                ...
                <div style={{ fontSize: 12, color: "#64748b", marginBottom: 4, display: "flex", alignItems: "center", gap: 6 }}>
                  <span>👨‍✈️ Driver:</span>
                  {isEditing
                    ? <input value={r.driver || ""} onChange={e => updateRoute(route, "driver", e.target.value)} style={{ border: "1.5px solid #c4b5fd", borderRadius: 6, padding: "3px 8px", fontSize: 12, fontFamily: "Georgia,serif", width: 180 }} />
                ...
                <div style={{ fontSize: 12, color: "#64748b", marginBottom: 4, display: "flex", alignItems: "center", gap: 6 }}>
                  <span>🚌 Vehicle:</span>
                  {isEditing
                    ? <input value={r.vehicle || ""} onChange={e => updateRoute(route, "vehicle", e.target.value)} style={{ border: "1.5px solid #c4b5fd", borderRadius: 6, padding: "3px 8px", fontSize: 12, fontFamily: "Georgia,serif", width: 160 }} />
                ...
                <div style={{ fontSize: 12, color: "#64748b", marginBottom: 4, display: "flex", alignItems: "center", gap: 6 }}>
                  <span>💺 Capacity:</span>
                  {isEditing
                    ? <input type="number" min={1} max={100} value={cap} onChange={e => updateRoute(route, "capacity", Math.min(100, Math.max(1, parseInt(e.target.value) || 40)))} style={{ border: "1.5px solid #c4b5fd", borderRadius: 6, padding: "3px 8px", fontSize: 12, fontFamily: "Georgia,serif", width: 80 }} />
                    : <b>{cap} learners</b>}
                </div>
                {/* Fare */}
                <div style={{ fontSize: 12, color: "#64748b", marginBottom: 4, display: "flex", alignItems: "center", gap: 6 }}>
                  <span>💰 Term Fare (KES):</span>
                  {isEditing
                    ? <input type="number" min={0} value={r.fare||15000} onChange={e => updateRoute(route, "fare", parseInt(e.target.value)||0)} style={{ border: "1.5px solid #c4b5fd", borderRadius: 6, padding: "3px 8px", fontSize: 12, fontFamily: "Georgia,serif", width: 100 }} />
                    : <b>KES {(r.fare||15000).toLocaleString()}</b>}
                </div>
                {/* Color */}
                {isEditing && <div style={{ fontSize: 12, color: "#64748b", marginBottom: 4, display: "flex", alignItems: "center", gap: 8 }}>
                  <span>🎨 Route Color:</span>
                  {PALETTE.map(c=><button key={c} onClick={()=>updateRoute(route,"color",c)} style={{width:20,height:20,borderRadius:"50%",background:c,border:r.color===c?"3px solid #000":"2px solid #ddd",cursor:"pointer",padding:0}}/>)}
                </div>}
                ...
                <div style={{ fontSize: 12, color: "#64748b", marginBottom: 4, display: "flex", alignItems: "center", gap: 6 }}>
                  ...
                  {isEditing
                    ? <input type="time" value={r.timeDeparted || ""} onChange={e => updateRoute(route, "timeDeparted", e.target.value)} style={{ border: "1.5px solid #c4b5fd", borderRadius: 6, padding: "3px 8px", fontSize: 12, fontFamily: "Georgia,serif" }} />
                    : <b>{r.timeDeparted || "—"}</b>}
                </div>
                ...
                <div style={{ fontSize: 12, color: "#64748b", marginBottom: 8, display: "flex", alignItems: "center", gap: 6 }}>
                  ...
                  {isEditing
                    ? <input type="time" value={r.timeArrived || ""} onChange={e => updateRoute(route, "timeArrived", e.target.value)} style={{ border: "1.5px solid #c4b5fd", borderRadius: 6, padding: "3px 8px", fontSize: 12, fontFamily: "Georgia,serif" }} />
                    : <b>{r.timeArrived || "—"}</b>}
                </div>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginBottom: 6 }}>
                  {(r.destinations || []).map((stop, i) => (
                    <span key={i} style={{ fontSize: 10, background: "#f8fafc", border: "1px solid #e2e8f0", padding: "3px 8px", borderRadius: 12, display: "flex", alignItems: "center", gap: 4 }}>
                      {isEditing
                        ? (editStopIdx === `${route}-${i}`
                          ? <input autoFocus value={stop} onChange={e => editStopName(route, i, e.target.value)} onBlur={() => setEditStopIdx(null)} style={{ border: "1px solid #c4b5fd", borderRadius: 4, padding: "1px 6px", fontSize: 11, fontFamily: "Georgia,serif", width: 110 }} />
                ...
                {isEditing && <div style={{ display: "flex", gap: 6, marginTop: 4 }}>
                  <input value={newStop} onChange={e => setNewStop(e.target.value)} onKeyDown={e => e.key === "Enter" && addStop(route)} placeholder="Add stop..." style={{ border: "1.5px solid #c4b5fd", borderRadius: 6, padding: "3px 8px", fontSize: 12, fontFamily: "Georgia,serif", flex: 1 }} />
                  <button onClick={() => addStop(route)} style={{ background: "#7c3aed", color: "white", border: "none", borderRadius: 6, padding: "3px 10px", cursor: "pointer", fontSize: 12, fontFamily: "Georgia,serif" }}>+ Add</button>
                </div>}
              </div>

              {/* Right panel: stats + edit button */}
              <div style={{ textAlign: "center", minWidth: 100 }}>
                <div style={{ fontWeight: "bold", color: routeColor(route), fontSize: 24 }}>{count}</div>
      ...
      {/* Add new route */}
      {tab === "routes" && user.role === "admin" && <Card style={{marginTop:14,borderLeft:"4px dashed #c4b5fd"}}>
        <div style={{fontWeight:"bold",color:"#3b0764",marginBottom:10,fontSize:13}}>➕ Add New Route</div>
        <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
          <input value={newRouteName} onChange={e=>setNewRouteName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addRoute()} placeholder="Route name (e.g. Route D, Chuka Route...)" style={{flex:1,minWidth:200,border:"1.5px solid #c4b5fd",borderRadius:8,padding:"7px 10px",fontSize:13,fontFamily:"Georgia,serif",outline:"none"}}/>
          <Btn onClick={addRoute} v="primary" style={{fontSize:12}}>Add Route</Btn>
        </div>
        <div style={{fontSize:11,color:"#94a3b8",marginTop:6}}>Tip: Click ✏️ next to any route name to rename it. Click "🗑️ Delete Route" to remove (only if no students assigned).</div>
      </Card>}

      {/* ── ASSIGN STUDENTS ── */}
      {tab === "assign" && user.role === "admin" && <Card>
        <div style={{ fontWeight: "bold", color: "#3b0764", marginBottom: 14, fontSize: 14 }}>Assign Student to Bus Route</div>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "flex-end" }}>
          ...
          <div style={{ flex: 2, minWidth: 220 }}>
            ...
            <select value={assignId} onChange={e => setAssignId(e.target.value)} style={{ width: "100%", border: "1.5px solid #e2e8f0", borderRadius: 8, padding: "8px 10px", fontSize: 13, fontFamily: "Georgia,serif" }}>
              <option value="">-- Select student --</option>
              {sortStudentsByAdm(students.filter(s => s.status !== "transferred" && (s.studentType||"Day Scholar")==="Day Scholar" && (tpGrade==="All"||s.class===tpGrade))).map(s => <option key={s.id} value={s.id}>{s.name} ({s.admNo||"—"}) — {s.class} {s.busRoute ? `[On: ${s.busRoute}]` : "[Unassigned]"}</option>)}
            </select>
          </div>
          <Btn onClick={assignStudent} v="blue">Assign to {selRoute}</Btn>
        </div>
        <div style={{ marginTop: 12, fontSize: 12, color: "#64748b" }}>
          {ROUTES.map(r => { const c = busStudents(r).length; const cap = transportRoutes[r]?.capacity || 40; return <span key={r} style={{ marginRight: 16 }}><b style={{ color: routeColor(r) }}>{r}:</b> {c}/{cap} seats used</span>; })}
        </div>
      </Card>}
      ...
      {tab === "roster" && <>
        <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
          {ROUTES.map(r => <Btn key={r} onClick={() => setSelRoute(r)} v={selRoute === r ? "primary" : "ghost"} style={{ fontSize: 12, borderLeft: `3px solid ${routeColor(r)}` }}>{r} ({busStudents(r).length})</Btn>)}
        </div>
        <Card style={{ padding: 0, overflow: "hidden" }}>
          <div style={{ padding: "12px 16px", background: routeColor(selRoute) + "15", fontWeight: "bold", color: routeColor(selRoute), fontSize: 13, borderBottom: "1px solid #e2e8f0" }}>
      ...
      {tab === "monitoring" && <>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, flexWrap: "wrap", gap: 10 }}>
          <div style={{ fontWeight: "bold", color: "#3b0764", fontSize: 14 }}>📡 Bus Movement Log</div>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
          ...
          {ROUTES.map(r => <Btn key={r} onClick={() => setSelRoute(r)} v={selRoute === r ? "primary" : "ghost"} style={{ fontSize: 11, borderLeft: `3px solid ${routeColor(r)}` }}>{r}</Btn>)}
        </div>
            ...
            <tbody>
              {(() => {
                ...
                return filtered.length ? filtered.map((m, i) => <tr key={m.id} style={{ background: i % 2 === 0 ? "white" : "#fafafa" }}>
                  ...
                  <td style={td}><span style={{ fontSize: 10, fontWeight: "bold", color: routeColor(m.route), background: routeColor(m.route) + "15", padding: "2px 8px", borderRadius: 12 }}>{m.route}</span></td>
...
// ══════════════════════════════════════════════════════════
// PAYROLL MODULE — KRA-compliant (PAYE, NSSF, SHIF, AHL)
// ══════════════════════════════════════════════════════════
function PayrollPage({ staff, user }) {
  const F = getAppFont();
  const blank = { staffId: "", month: new Date().toLocaleString("en-KE",{month:"long"}), year: String(new Date().getFullYear()), basicSalary: "", houseAllowance: "", transportAllowance: "", otherAllowance: "", otherDeduction: "", notes: "" };
  const MONTHS = ["January","February","March","April","May","June","July","August","September","October","November","December"];
  ...
  useEffect(() => {
    load("tnks_payroll").then(d => { if (d) setRecords(d); });
  }, []);
  useEffect(() => { save("tnks_payroll", records); }, [records]);

  function flash(t) { setMsg(t); setTimeout(() => setMsg(""), 3000); }

  function calcPayroll(basic, house, transport, otherAllow, otherDed) {
    const b = parseFloat(basic) || 0;
    const ha = parseFloat(house) || 0;
    const ta = parseFloat(transport) || 0;
    const oa = parseFloat(otherAllow) || 0;
    const grossSalary = b + ha + ta + oa;
    // NSSF Tier I (up to 7,000 @ 6%) + Tier II (7,001–36,000 @ 6%)
    const nssfTier1 = Math.min(b, 7000) * 0.06;
    const nssfTier2 = Math.max(0, Math.min(b, 36000) - 7000) * 0.06;
    const nssf = nssfTier1 + nssfTier2;
    // SHIF — 2.75% of gross
    const shif = grossSalary * 0.0275;
    // Affordable Housing Levy — 1.5% employee
    const ahl = grossSalary * 0.015;
    // Taxable pay (gross - NSSF)
    const taxable = Math.max(0, grossSalary - nssf);
    // PAYE — KRA 2024 bands (monthly)
    let paye = 0;
    if (taxable <= 24000) paye = taxable * 0.10;
    else if (taxable <= 32333) paye = 2400 + (taxable - 24000) * 0.25;
    else if (taxable <= 500000) paye = 4483.25 + (taxable - 32333) * 0.30;
    else if (taxable <= 800000) paye = 144736.75 + (taxable - 500000) * 0.325;
    else paye = 242236.75 + (taxable - 800000) * 0.35;
    // Personal relief
    paye = Math.max(0, paye - 2400);
    const totalDed = nssf + shif + ahl + paye + (parseFloat(otherDed) || 0);
    const netPay = grossSalary - totalDed;
    return { grossSalary, nssf: nssf.toFixed(2), shif: shif.toFixed(2), ahl: ahl.toFixed(2), paye: paye.toFixed(2), totalDeductions: totalDed.toFixed(2), netPay: netPay.toFixed(2) };
  }

  function doProcess() {
    if (!form.staffId || !form.basicSalary) return flash("❌ Select staff and enter basic salary.");
    const s = staff.find(x => x.id === form.staffId);
    if (!s) return flash("❌ Staff not found.");
    const calc = calcPayroll(form.basicSalary, form.houseAllowance, form.transportAllowance, form.otherAllowance, form.otherDeduction);
    const rec = { id: Date.now().toString(), staffId: form.staffId, staffName: s.name, staffId2: s.staffId, month: form.month, year: form.year, basicSalary: parseFloat(form.basicSalary) || 0, houseAllowance: parseFloat(form.houseAllowance) || 0, transportAllowance: parseFloat(form.transportAllowance) || 0, otherAllowance: parseFloat(form.otherAllowance) || 0, otherDeduction: parseFloat(form.otherDeduction) || 0, notes: form.notes, ...calc, processedBy: user.name, processedOn: new Date().toLocaleDateString("en-KE") };
    setRecords(p => [...p.filter(r => !(r.staffId === form.staffId && r.month === form.month && r.year === form.year)), rec]);
    flash("✅ Payslip processed!");
    setForm(blank);
  }

  function printSlip(r) {
    const html = `<div style="font-family:Georgia,serif;max-width:600px;margin:auto;border:2px solid #3b0764;padding:28px;border-radius:12px;">
      <div style="text-align:center;border-bottom:3px double #3b0764;padding-bottom:12px;margin-bottom:16px;">
        <div style="font-size:20px;font-weight:bold;color:#3b0764;">${SCHOOL.name}</div>
        <div style="font-size:12px;color:#64748b;">${SCHOOL.location}</div>
        <div style="font-size:15px;font-weight:bold;margin-top:8px;color:#7c3aed;">PAYSLIP — ${r.month} ${r.year}</div>
      </div>
      <table style="width:100%;margin-bottom:16px;"><tr><td><b>Name:</b> ${r.staffName}</td><td><b>Staff ID:</b> ${r.staffId2||"—"}</td></tr><tr><td><b>Period:</b> ${r.month} ${r.year}</td><td><b>Processed:</b> ${r.processedOn}</td></tr></table>
      <table style="width:100%;border-collapse:collapse;margin-bottom:16px;">
        <thead><tr style="background:#3b0764;color:white;"><th style="padding:8px;text-align:left;">EARNINGS</th><th style="padding:8px;text-align:right;">KES</th></tr></thead>
        <tbody>
          <tr><td style="padding:6px 8px;">Basic Salary</td><td style="padding:6px 8px;text-align:right;">${Number(r.basicSalary).toLocaleString()}</td></tr>
          <tr><td style="padding:6px 8px;">House Allowance</td><td style="padding:6px 8px;text-align:right;">${Number(r.houseAllowance).toLocaleString()}</td></tr>
          <tr><td style="padding:6px 8px;">Transport Allowance</td><td style="padding:6px 8px;text-align:right;">${Number(r.transportAllowance).toLocaleString()}</td></tr>
          ${r.otherAllowance > 0 ? `<tr><td style="padding:6px 8px;">Other Allowances</td><td style="padding:6px 8px;text-align:right;">${Number(r.otherAllowance).toLocaleString()}</td></tr>` : ""}
          <tr style="font-weight:bold;background:#f5f3ff;"><td style="padding:8px;">GROSS SALARY</td><td style="padding:8px;text-align:right;">KES ${Number(r.grossSalary).toLocaleString()}</td></tr>
        </tbody>
      </table>
      <table style="width:100%;border-collapse:collapse;margin-bottom:16px;">
        <thead><tr style="background:#b91c1c;color:white;"><th style="padding:8px;text-align:left;">DEDUCTIONS</th><th style="padding:8px;text-align:right;">KES</th></tr></thead>
        <tbody>
          <tr><td style="padding:6px 8px;">PAYE (Income Tax)</td><td style="padding:6px 8px;text-align:right;">${Number(r.paye).toLocaleString()}</td></tr>
          <tr><td style="padding:6px 8px;">NSSF</td><td style="padding:6px 8px;text-align:right;">${Number(r.nssf).toLocaleString()}</td></tr>
          <tr><td style="padding:6px 8px;">SHIF (2.75%)</td><td style="padding:6px 8px;text-align:right;">${Number(r.shif).toLocaleString()}</td></tr>
          <tr><td style="padding:6px 8px;">Housing Levy (1.5%)</td><td style="padding:6px 8px;text-align:right;">${Number(r.ahl).toLocaleString()}</td></tr>
          ${r.otherDeduction > 0 ? `<tr><td style="padding:6px 8px;">Other Deductions</td><td style="padding:6px 8px;text-align:right;">${Number(r.otherDeduction).toLocaleString()}</td></tr>` : ""}
          <tr style="font-weight:bold;background:#fee2e2;"><td style="padding:8px;">TOTAL DEDUCTIONS</td><td style="padding:8px;text-align:right;">KES ${Number(r.totalDeductions).toLocaleString()}</td></tr>
        </tbody>
      </table>
      <div style="background:#3b0764;color:white;padding:14px 18px;border-radius:8px;display:flex;justify-content:space-between;align-items:center;">
        <span style="font-size:16px;font-weight:bold;">NET PAY</span>
        <span style="font-size:20px;font-weight:bold;">KES ${Number(r.netPay).toLocaleString()}</span>
      </div>
      ${r.notes ? `<div style="margin-top:12px;font-size:12px;color:#64748b;">Notes: ${r.notes}</div>` : ""}
      <div style="margin-top:20px;display:flex;justify-content:space-between;font-size:11px;color:#94a3b8;">
        <div>Processed by: ${r.processedBy}</div><div>© ${SCHOOL.name}</div>
      </div>
    </div>`;
    const w = window.open("", "_blank"); w.document.write(`<html><head><title>Payslip</title></head><body>${html}<script>window.onload=()=>{window.print();}<\/script></body></html>`); w.document.close();
  }

  const th = { textAlign:"left", padding:"9px 12px", fontWeight:"bold", fontSize:11, color:"#7c3aed", background:"#f5f3ff" };
  const td = { padding:"9px 12px", fontSize:12, borderBottom:"1px solid #f1f5f9", fontFamily:F };
  const selMonth = form.month;
  const selYear = form.year;
  const filtered = records.filter(r => r.month === selMonth && r.year === selYear);
  const totalGross = filtered.reduce((a,r) => a + (parseFloat(r.grossSalary)||0), 0);
  const totalNet = filtered.reduce((a,r) => a + (parseFloat(r.netPay)||0), 0);

  return (
    <div style={{padding:20,fontFamily:F}}>
      <PageH title="💼 Payroll Manager" sub="KRA-compliant: PAYE · NSSF · SHIF · Housing Levy">
        {["process","records","summary"].map(t => (
          <Btn key={t} v={tab===t?"purple":"ghost"} onClick={()=>setTab(t)} style={{fontSize:12,textTransform:"capitalize"}}>{t==="process"?"➕ Process Payslip":t==="records"?"📋 Records":"📊 Summary"}</Btn>
        ))}
      </PageH>
      {tab==="process"&&(
        <Card>
          <div style={{fontWeight:"bold",color:"#3b0764",marginBottom:16,fontSize:14}}>Process New Payslip</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:12,marginBottom:16}}>
            <div><label style={{fontSize:11,fontWeight:"bold",color:"#374151",display:"block",marginBottom:4}}>STAFF MEMBER *</label>
              <select value={form.staffId} onChange={e=>setForm({...form,staffId:e.target.value})} style={{width:"100%",border:"1.5px solid #e2e8f0",borderRadius:8,padding:"8px",fontSize:13,fontFamily:F}}>
                <option value="">-- Select staff --</option>
                {staff.map(s=><option key={s.id} value={s.id}>{s.name} ({s.staffId||"—"})</option>)}
              </select>
            </div>
            <Sel label="MONTH" value={form.month} onChange={v=>setForm({...form,month:v})} options={MONTHS}/>
            <Sel label="YEAR" value={form.year} onChange={v=>setForm({...form,year:v})} options={YEARS}/>
          </div>
          <div style={{fontWeight:"bold",color:"#374151",marginBottom:10,fontSize:12}}>💰 EARNINGS</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:12,marginBottom:16}}>
            <Inp label="BASIC SALARY (KES) *" value={form.basicSalary} onChange={v=>setForm({...form,basicSalary:v})} placeholder="e.g. 25000" type="number"/>
            <Inp label="HOUSE ALLOWANCE (KES)" value={form.houseAllowance} onChange={v=>setForm({...form,houseAllowance:v})} placeholder="0" type="number"/>
            <Inp label="TRANSPORT ALLOW. (KES)" value={form.transportAllowance} onChange={v=>setForm({...form,transportAllowance:v})} placeholder="0" type="number"/>
            <Inp label="OTHER ALLOWANCES (KES)" value={form.otherAllowance} onChange={v=>setForm({...form,otherAllowance:v})} placeholder="0" type="number"/>
          </div>
          <div style={{fontWeight:"bold",color:"#374151",marginBottom:10,fontSize:12}}>📉 ADDITIONAL DEDUCTIONS</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:12,marginBottom:16}}>
            <Inp label="OTHER DEDUCTIONS (KES)" value={form.otherDeduction} onChange={v=>setForm({...form,otherDeduction:v})} placeholder="e.g. SACCO loan" type="number"/>
            <Inp label="NOTES" value={form.notes} onChange={v=>setForm({...form,notes:v})} placeholder="Optional note"/>
          </div>
          {form.basicSalary&&(()=>{const c=calcPayroll(form.basicSalary,form.houseAllowance,form.transportAllowance,form.otherAllowance,form.otherDeduction);return(
      ...
      {tab==="records"&&(
        <>
          <div style={{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap",alignItems:"center"}}>
            <Sel value={selMonth} onChange={v=>setForm(p=>({...p,month:v}))} options={MONTHS}/>
            <Sel value={selYear} onChange={v=>setForm(p=>({...p,year:v}))} options={YEARS}/>
            <span style={{fontSize:12,color:"#64748b"}}>{filtered.length} payslip(s)</span>
          </div>
          <Card style={{padding:0}}>
            <div style={{overflowX:"auto"}}>
              <table style={{width:"100%",borderCollapse:"collapse",minWidth:700}}>
                <thead><tr>{["Staff","Month","Gross","PAYE","NSSF","SHIF","AHL","Net Pay","Actions"].map(h=><th key={h} style={th}>{h}</th>)}</tr></thead>
                <tbody>{filtered.length?filtered.map((r,i)=>(
                  <tr key={r.id} style={{background:i%2===0?"white":"#fafafa"}}>
                    <td style={{...td,fontWeight:"bold"}}>{r.staffName}</td>
                    <td style={td}>{r.month} {r.year}</td>
                    <td style={{...td,color:"#3b0764",fontWeight:"bold"}}>KES {Number(r.grossSalary).toLocaleString()}</td>
                    <td style={{...td,color:"#b91c1c"}}>KES {Number(r.paye).toLocaleString()}</td>
      ...
      {tab==="summary"&&(
        <>
          ...
          {filtered.length>0&&(
            <Card>
              <div style={{fontWeight:"bold",color:"#3b0764",marginBottom:14,fontSize:14}}>Payroll Summary — {selMonth} {selYear}</div>
              <div style={{overflowX:"auto"}}>
                <table style={{width:"100%",borderCollapse:"collapse",minWidth:600}}>
                  <thead><tr>{["#","Staff Name","Gross","Total Ded.","Net Pay"].map(h=><th key={h} style={th}>{h}</th>)}</tr></thead>
          ...
          {!filtered.length&&<Empty icon="💼" text={`No payroll processed for ${selMonth} ${selYear}`}/>}
        </>
      )}
    </div>
  );
...
function PocketMoneyPage({ students, user }) {
  ...
  useEffect(() => { load("tnks_pocket_money").then(d => { if (d) setAccounts(d); }); }, []);
  useEffect(() => { save("tnks_pocket_money", accounts); }, [accounts]);

  function flash(t, ok=true) { setMsg({t,ok}); setTimeout(()=>setMsg(""),3000); }

  function getBalance(stuId) {
    const txs = accounts.filter(a => a.studentId === stuId);
    return txs.reduce((sum, t) => sum + (t.type==="Deposit"?1:-1) * (parseFloat(t.amount)||0), 0);
  }

  function doTransaction() {
    if (!selStu || !txAmount) return flash("Select student and enter amount.", false);
    const amt = parseFloat(txAmount);
    if (isNaN(amt) || amt <= 0) return flash("Enter a valid positive amount.", false);
    if (txType === "Withdrawal") {
      const bal = getBalance(selStu);
      if (amt > bal) return flash(`Insufficient balance. Current: KES ${bal.toLocaleString()}`, false);
    }
    const s = students.find(x => x.id === selStu);
    const tx = { id: Date.now().toString(), studentId: selStu, studentName: s?.name||"—", studentClass: s?.class||"—", type: txType, amount: amt, note: txNote, date: new Date().toLocaleDateString("en-KE"), time: new Date().toLocaleTimeString("en-KE",{hour:"2-digit",minute:"2-digit"}), recordedBy: user.name };
    setAccounts(p => [...p, tx]);
    flash(`✅ ${txType} of KES ${amt.toLocaleString()} recorded!`);
    setTxAmount(""); setTxNote(""); setSelStu("");
  }

  const stuWithAccts = students.filter(s => accounts.some(a => a.studentId === s.id));
  const allStus = students.filter(s => !search || s.name.toLowerCase().includes(search.toLowerCase()) || s.admNo?.toLowerCase().includes(search.toLowerCase()));
  const th = { textAlign:"left", padding:"9px 12px", fontWeight:"bold", fontSize:11, color:"#7c3aed", background:"#f5f3ff" };
  const td = { padding:"9px 12px", fontSize:12, borderBottom:"1px solid #f1f5f9", fontFamily:F };

  return (
    <div style={{padding:20,fontFamily:F}}>
      <PageH title="🪙 Pocket Money" sub="Student digital wallet — deposits, withdrawals & balances">
        {["accounts","transact","history"].map(t=>(
          <Btn key={t} v={tab===t?"purple":"ghost"} onClick={()=>setTab(t)} style={{fontSize:12}}>
            {t==="accounts"?"💳 Balances":t==="transact"?"➕ Transaction":"📋 History"}
          </Btn>
        ))}
      </PageH>
      {tab==="transact"&&(
        <Card style={{marginBottom:20}}>
          <div style={{fontWeight:"bold",color:"#3b0764",marginBottom:16,fontSize:14}}>Record Transaction</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:12,marginBottom:12}}>
            ...
            <Inp label="AMOUNT (KES) *" value={txAmount} onChange={setTxAmount} placeholder="e.g. 500" type="number"/>
            <Inp label="NOTE / DESCRIPTION" value={txNote} onChange={setTxNote} placeholder="e.g. weekly allowance"/>
          </div>
          {selStu&&(
            <div style={{background:"#f5f3ff",borderRadius:8,padding:"10px 14px",marginBottom:12,fontSize:13}}>
      ...
      {tab==="accounts"&&(
        <>
          <div style={{display:"flex",gap:10,marginBottom:14,flexWrap:"wrap",alignItems:"center"}}>
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search student..." style={{flex:1,minWidth:200,border:"1.5px solid #e2e8f0",borderRadius:9,padding:"8px 12px",fontSize:13,fontFamily:F,outline:"none"}}/>
            <span style={{fontSize:12,color:"#64748b"}}>{stuWithAccts.length} student(s) with accounts</span>
          </div>
          {students.length===0&&<Empty icon="🪙" text="No students enrolled yet."/>}
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:14}}>
            {allStus.map(s=>{
              const bal = getBalance(s.id);
              const txCount = accounts.filter(a=>a.studentId===s.id).length;
              return (
                <Card key={s.id} style={{padding:16,cursor:"pointer",border:`1.5px solid ${bal>0?"#ddd6fe":"#e2e8f0"}`}} onClick={()=>{setSelStu(s.id);setTab("history");}}>
                  <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
                    <Avatar name={s.name} photo={s.photo} size={38}/>
                    <div><div style={{fontWeight:"bold",fontSize:13}}>{s.name}</div><div style={{fontSize:11,color:"#64748b"}}>{s.class} · {s.admNo}</div></div>
                  </div>
                  <div style={{background: bal>0?"#f5f3ff":"#f1f5f9",borderRadius:8,padding:"10px 12px",textAlign:"center"}}>
                    <div style={{fontSize:11,color:"#64748b"}}>Balance</div>
                    <div style={{fontSize:18,fontWeight:"bold",color:bal>0?"#3b0764":"#94a3b8"}}>KES {bal.toLocaleString()}</div>
                  </div>
                  <div style={{fontSize:11,color:"#94a3b8",textAlign:"center",marginTop:6}}>{txCount} transaction(s)</div>
                </Card>
              );
            })}
          </div>
      ...
      {tab==="history"&&(
        <>
          <div style={{display:"flex",gap:10,marginBottom:14,flexWrap:"wrap",alignItems:"center"}}>
            <div style={{flex:1}}><select value={selStu} onChange={e=>setSelStu(e.target.value)} style={{width:"100%",border:"1.5px solid #e2e8f0",borderRadius:8,padding:"8px 12px",fontSize:13,fontFamily:F}}>
              <option value="">-- All students --</option>
              {students.map(s=><option key={s.id} value={s.id}>{s.name} — {s.class}</option>)}
            </select></div>
            {selStu&&<div style={{fontWeight:"bold",color:"#3b0764",fontSize:14}}>Balance: KES {getBalance(selStu).toLocaleString()}</div>}
          </div>
          <Card style={{padding:0}}>
            <div style={{overflowX:"auto"}}>
              <table style={{width:"100%",borderCollapse:"collapse",minWidth:500}}>
                <thead><tr>{["Date","Student","Class","Type","Amount","Note","By"].map(h=><th key={h} style={th}>{h}</th>)}</tr></thead>
                <tbody>{(selStu?accounts.filter(a=>a.studentId===selStu):accounts).slice().reverse().map((a,i)=>(
                  <tr key={a.id} style={{background:i%2===0?"white":"#fafafa"}}>
                ...
                {accounts.length===0&&<tr><td colSpan={7} style={{padding:40,textAlign:"center",color:"#94a3b8"}}>No transactions yet.</td></tr>}
                </tbody>
              </table>
            </div>
          </Card>
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      {tab === "add" && user.role === "admin" && <Card style={{ marginBottom: 16 }}>
        <div style={{ fontWeight: "bold", color: "#3b0764", marginBottom: 14, fontSize: 14 }}>{editId ? "Edit Item" : "Add New Inventory Item"}</div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(170px,1fr))", gap: 12 }}>
              ...
              return <tr key={x.id} style={{ background: isLow ? "#fef9f9" : i % 2 === 0 ? "white" : "#fafafa" }}>
                ...
                {user.role === "admin" && <td style={td}>
                  <div style={{ display: "flex", gap: 4 }}>
                    <button onClick={() => setAdjId(x.id)} style={{ background: "#f5f3ff", color: "#7c3aed", border: "none", borderRadius: 6, padding: "3px 8px", cursor: "pointer", fontSize: 11, fontFamily: "Georgia,serif" }}>±</button>
                    <button onClick={() => { setEditId(x.id); setForm({ name: x.name, category: x.category, unit: x.unit, quantity: x.quantity, minStock: x.minStock, unitCost: x.unitCost, supplier: x.supplier || "", location: x.location || "", notes: x.notes || "" }); setTab("add"); }} style={{ color: "#7c3aed", background: "none", border: "none", cursor: "pointer", fontSize: 11 }}>Edit</button>
...
// SUBJECT ALLOCATION
// ══════════════════════════════════════════════════════════
function SubjectAllocationPage({ staff, user, students, ttSetup }) {
  const F = getAppFont();
  const subjectMap = ttSetup?.setupData?.subjectTeachers || PRELOADED_TEACHERS;

  // Build a flat list from the timetable: {cls, subject, teacherName}
  // Also resolve combined results subjects to show them grouped
  const rows = [];
  ALL_CLASSES.forEach(cls => {
    const resultsSubs = getSubs(cls);
    resultsSubs.forEach(sub => {
      const parts = COMBINED_SUBJECT_PARTS[sub] || [sub];
      // Collect teacher names from all component timetable subjects (support arrays)
      const teachers = [...new Set(parts.flatMap(p => getTeachersForKey(subjectMap, `${cls}::${p}`)).filter(Boolean))];
      rows.push({ cls, subject: sub, teachers: teachers.length ? teachers : ["—"], parts });
    });
  });
  ...
  const allTeachers = [...new Set(Object.values(subjectMap))].sort();
  const visible = rows.filter(r =>
    (filterCls === "All" || r.cls === filterCls) &&
    (filterTeacher === "All" || r.teachers.includes(filterTeacher))
  );
  ...
  return (
    <div style={{padding:24}}>
      <PageH title="📚 Subject Allocation" sub="Teacher assignments are read directly from the Timetable — no manual entry needed"/>

      {/* Info banner */}
      <div style={{background:"#eff6ff",border:"1px solid #bfdbfe",borderRadius:10,padding:"12px 16px",marginBottom:18,display:"flex",gap:10,alignItems:"flex-start"}}>
        <span style={{fontSize:18}}>ℹ️</span>
        <div style={{fontSize:13,color:"#1e40af"}}>
          <b>Assignments come from your Timetable.</b> To change which teacher handles a subject, go to
          <b> Timetable → Setup → Subject Teachers</b> and update it there.
          This page automatically reflects whatever is set in the timetable — no separate data entry required.
        </div>
      </div>
      ...
      <Card style={{marginBottom:16}}>
        <div style={{display:"flex",gap:12,flexWrap:"wrap",alignItems:"flex-end"}}>
          <Sel label="FILTER BY CLASS" value={filterCls} onChange={setFilterCls} options={["All",...ALL_CLASSES]}/>
          <Sel label="FILTER BY TEACHER" value={filterTeacher} onChange={setFilterTeacher}
            options={["All",...allTeachers]} labels={["All Teachers",...allTeachers]}/>
          <div style={{alignSelf:"flex-end",fontSize:12,color:"#64748b",paddingBottom:8}}>
            Showing <b>{visible.length}</b> assignments
          </div>
        </div>
      </Card>
      ...
      {/* Table */}
      <Card style={{padding:0}}>
        <div style={{padding:"12px 16px",background:"#f5f3ff",fontWeight:"bold",color:"#3b0764",fontSize:13,borderBottom:"1px solid #ede9fe",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <span>Subject → Teacher Map (from Timetable)</span>
          <span style={{fontSize:11,color:"#7c3aed",fontWeight:"normal"}}>{Object.keys(subjectMap).length} timetable entries</span>
        </div>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",minWidth:500}}>
            <thead><tr>
              ...
              <th style={th}>RESULTS SUBJECT</th>
              <th style={th}>ASSIGNED TEACHER(S)</th>
              <th style={th}>TIMETABLE SUBJECT(S)</th>
            </tr></thead>
            <tbody>{visible.length ? visible.map((r,i) => (
              <tr key={i} style={{background:i%2===0?"white":"#fafafa"}}>
                <td style={{...td,color:"#94a3b8"}}>{i+1}</td>
                <td style={td}><span style={{fontSize:11,background:"#f5f3ff",color:"#7c3aed",padding:"2px 8px",borderRadius:20,fontWeight:"bold"}}>{r.cls}</span></td>
                <td style={{...td,fontWeight:"bold",color:"#1e293b"}}>{r.subject}</td>
                <td style={td}>{r.teachers.map((t,j) => (
                  <span key={j} style={{display:"inline-block",background: t==="—"?"#fef2f2":"#f0fdf4",color:t==="—"?"#b91c1c":"#166534",fontSize:11,padding:"2px 10px",borderRadius:20,fontWeight:"bold",marginRight:4,marginBottom:2}}>{t}</span>
                ))}</td>
                <td style={{...td,color:"#64748b",fontSize:11}}>
                  {r.parts.length===1&&r.parts[0]===r.subject ? "—" : r.parts.join(" + ")}
                </td>
              </tr>
            )) : <tr><td colSpan={5} style={{padding:30,textAlign:"center",color:"#94a3b8"}}>No assignments found for this filter.</td></tr>}
            </tbody>
          </table>
        </div>
      </Card>
...
function EditMarksPage({ students, results, setResults, term, year, examType }) {
  ...
  function getSubjects(cls) { return getSubs(cls); }
...
function StudentRoutesPage({ students, setStudents, user }) {
  ...
  const unassigned = students.filter(s => !s.busRoute && (s.studentType||"Day Scholar")==="Day Scholar" && s.status!=="transferred");
...
export default function App(){
  ...
  // Global: Enter key on any input/select moves focus to the next focusable field.
  // This makes the blue "go" arrow on mobile keyboards advance the cursor app-wide.
  useEffect(()=>{
    function onEnterAdvance(e){
      if(e.key!=="Enter") return;
      const tag=e.target.tagName;
      // Only intercept on inputs & selects — not textareas (Enter = newline) or buttons (Enter = click)
      if(tag!=="INPUT"&&tag!=="SELECT") return;
      // Allow <input type="submit|button|reset"> to fire normally
      const t=e.target.type||"";
      if(t==="submit"||t==="button"||t==="reset") return;
      // Collect all focusable form fields in tab order
      const all=[...document.querySelectorAll(
        'input:not([disabled]):not([type="hidden"]),select:not([disabled]),textarea:not([disabled]),button:not([disabled])'
      )].filter(el=>el.offsetParent!==null&&!el.closest("[aria-hidden='true']"));
      const idx=all.indexOf(e.target);
      if(idx===-1) return;
      // Find the next input/select (skip buttons so Enter doesn't accidentally submit)
      for(let i=idx+1;i<all.length;i++){
        const next=all[i];
        if(next.tagName==="BUTTON") continue;
        e.preventDefault();
        next.focus();
        // For number/text inputs, select text so user can type over it
...
const [users,setUsers]=useState(DEFAULT_USERS);
  ...
  useEffect(()=>{
    (async()=>{
      try{
        const [u,s,r,c,a,f,st,lg,mon,tt,tts,bk,br,ev,co,sd,du,ta,sr,fs,es,cl,pc,inv,tr,bmon,pr,pm,examCfg]=await Promise.all([
          ...
          load("tnks_payroll"),load("tnks_pocket_money"),
          load("tnks_exam_settings"),
        ]);
        if(u){
        ...
        // ── Auto-apply admin/dean exam settings so teachers don't need to filter ──
        if(examCfg){
          if(examCfg.currentTerm)setTerm(examCfg.currentTerm);
          if(examCfg.currentYear)setYear(examCfg.currentYear);
          if(examCfg.currentExam)setExamType(examCfg.currentExam);
        }
  ...
  useEffect(()=>{if(ready)save("tnks_payroll",payroll);},[payroll,ready]);
  useEffect(()=>{if(ready)save("tnks_pocket_money",pocketMoney);},[pocketMoney,ready]);
  ...
  const ctx={user,students,setStudents,results,setResults,comments,setComments,announcements,setAnnouncements,users,setUsers,fees,setFees,staff,setStaff,term,setTerm,year,setYear,examType,setExamType,logo,monitoring,setMonitoring,ttSetup};
  ...
  return(
    <div style={{display:"flex",height:"100vh",fontFamily:_appFont2,background:_bg2,position:"relative"}}>
      ...
      <main style={{flex:1,overflowY:"auto",overflowX:"auto",minWidth:0}}>
        ...
        {view==="ai_comments"&&<AICommentAssistant students={students} results={results} comments={comments} setComments={setComments} term={term} year={year} examType={examType}/>}
        {view==="inventory"&&<InventoryPage user={user} inventory={inventory} setInventory={setInventory}/>}
        {view==="payroll"&&user.role==="admin"&&<PayrollPage staff={staff} user={user}/>}
        {view==="payroll"&&user.role!=="admin"&&<div style={{padding:40,textAlign:"center",color:"#94a3b8"}}>Admin access required.</div>}
        {view==="pocketmoney"&&<PocketMoneyPage students={students} user={user}/>}
        {view==="messages"&&<MessagesPage user={user}/>}
        {view==="settings"&&user.role==="admin"&&<SettingsPage users={users} setUsers={setUsers} logo={logo} setLogo={setLogo} ttSetup={ttSetup} setTtSetup={setTtSetup}/>}
        ...
        {view==="subject_allocation"&&<SubjectAllocationPage staff={staff} user={user} students={students} ttSetup={ttSetup}/>}
