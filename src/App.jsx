...
function buildStampBox() {
  return `<div style="display:flex;justify-content:flex-end;margin-top:14px;">
    <div style="border:2px dashed #3b0764;border-radius:8px;width:160px;height:80px;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:6px;">
...
function buildHTMLDoc(title, bodyHTML, logo) {
  ...
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"/><title>${title}</title><style>
    ...
    *{-webkit-print-color-adjust:exact !important;print-color-adjust:exact !important;color-adjust:exact !important;}
      ...
      .results-table tbody tr, .grade-table tbody tr{
        height:32px;
      }
      ...
      .results-table th, .grade-table th{
  ...
  function buildClassResultsHTML(classStudents, className) {
    ...
    const rows=ranked.map((s,i)=>{
      ...
      return `<tr style="background:${i%2===0?"rgba(255,255,255,0.55)":"rgba(248,250,252,0.55)"}">
        ...
        <td style="padding:6px 8px;font-weight:bold;white-space:nowrap;min-width:200px;max-width:220px;overflow:hidden;text-overflow:ellipsis;vertical-align:${i%2===0?"top":"bottom"};">${(s.name||"").trim().toUpperCase()}</td>
        ${subs.map(su=>{const isComb=su===SCI_AGRI_COMBINED;const v=getPrintMark(sr,su);return`<td style="padding:5px 4px;text-align:center;font-weight:bold;font-size:13px;${isComb?`background:${SCI_AGRI_BG};color:${SCI_AGRI_DARK};`:""}">${v!==null?(isComb?v.toFixed(1):v):"—"}</td>`;}).join("")}
        <td style="padding:5px 6px;text-align:center;font-weight:bold;font-size:13px;background:rgba(254,243,199,0.7);">${dispM.some(v=>v!==null)?dispTotal.toFixed(0):"—"}</td>
    ...
    return `<div style="margin-bottom:32px;">
      ...
      <table class="results-table" style="width:100%;border-collapse:collapse;font-size:10px;min-width:600px;">
        <thead><tr style="background:#3b0764;">
          ...
          <th style="padding:5px 8px;text-align:left;color:white;min-width:220px;">Name</th>
          ${subs.map(s=>{const isComb=s===SCI_AGRI_COMBINED;return`<th style="padding:0;width:42px;min-width:42px;height:150px;vertical-align:bottom;background:${isComb?"#0d9488":"#3b0764"};position:relative;overflow:visible;" title="${s}"><div style="position:absolute;bottom:10px;right:4px;transform:rotate(-45deg);transform-origin:right bottom;white-space:nowrap;font-size:11px;font-weight:bold;color:white;">${s}</div></th>`;}).join("")}
          <th style="padding:5px 6px;text-align:center;background:rgba(180,83,9,0.85);color:white;white-space:nowrap;">Total</th>
        ...
        <tfoot style="page-break-inside:avoid;">
          ...
          ${teacherRow}
          <tr style="border:none;"><td colspan="${subs.length+3}" style="border:none;padding:10px 0 4px 0;text-align:right;background:white;"><div style="display:inline-block;border:2px dashed #3b0764;border-radius:8px;width:160px;height:70px;text-align:center;padding:8px 6px;"><div style="font-size:9px;font-weight:bold;color:#3b0764;letter-spacing:0.5px;">OFFICIAL STAMP</div><div style="font-size:8px;color:#94a3b8;margin-top:14px;">Date: _______________</div></div></td></tr>
        </tfoot>
      </table></div>
    </div>`;
  }
  ...
  function buildClassGradesHTML(classStudents, className) {
    ...
    const rows=ranked.map((s,i)=>{
      ...
      return `<tr style="background:${i%2===0?"rgba(255,255,255,0.55)":"rgba(248,250,252,0.55)"}">
        <td style="padding:5px 6px;font-weight:bold;color:${i<3?"#b45309":"#374151"};text-align:center;">${s.position||"—"}</td>
        <td style="padding:6px 8px;font-weight:bold;white-space:nowrap;min-width:200px;max-width:220px;overflow:hidden;text-overflow:ellipsis;vertical-align:${i%2===0?"top":"bottom"};">${(s.name||"").trim().toUpperCase()}</td>
        ${subs.map(su=>{const isComb=su===SCI_AGRI_COMBINED;const v=getPrintMark(sr,su);const g=v!==null?getGrade(v):null;return`<td style="padding:5px 4px;text-align:center;${isComb?`background:${SCI_AGRI_BG};`:""}">${g?`<span style="background:${g.bg};color:${g.col};font-size:12px;padding:2px 5px;border-radius:6px;font-weight:bold;">${g.g}</span>`:"—"}</td>`;}).join("")}
        <td style="padding:5px 6px;text-align:center;">${og?`<span style="background:${og.bg};color:${og.col};font-size:12px;padding:2px 5px;border-radius:6px;font-weight:bold;">${og.g}</span>`:"—"}</td>
      </tr>`;
    }).join("");
    const teacherRow=`<tr style="background:#f5f3ff;border-top:2px solid #1d4ed8;">
      ...
      ${subs.map(s=>{const isComb=s===SCI_AGRI_COMBINED;const ts=isComb?[...new Set(["Integrated Science","Agriculture and Nutrition"].flatMap(p=>getSubjectTeachersForResult(className,p)).filter(Boolean))]:getSubjectTeachersForResult(className,s);const name=ts.length?ts.join(" / "):"—";return`<td style="padding:4px 3px;text-align:center;font-size:9px;color:${isComb?SCI_AGRI_DARK:"#1d4ed8"};font-weight:bold;white-space:normal;word-break:break-word;">${name}</td>`;}).join("")}
      <td style="padding:4px 6px;font-size:9px;color:#64748b;"></td>
    </tr>`;
    return `<div style="margin-bottom:32px;">
      ${buildSectionHeader(logo)}
      ...
      <table class="grade-table" style="width:100%;border-collapse:collapse;font-size:10px;min-width:600px;">
        <thead><tr style="background:#1d4ed8;">
          <th style="padding:5px 6px;text-align:center;color:white;">Pos</th>
          <th style="padding:5px 8px;text-align:left;color:white;min-width:220px;">Name</th>
          ${subs.map(s=>{const isComb=s===SCI_AGRI_COMBINED;return`<th style="padding:0;width:42px;min-width:42px;height:150px;vertical-align:bottom;background:${isComb?"#0d9488":"#1d4ed8"};position:relative;overflow:visible;" title="${s}"><div style="position:absolute;bottom:10px;right:4px;transform:rotate(-45deg);transform-origin:right bottom;white-space:nowrap;font-size:11px;font-weight:bold;color:white;">${s}</div></th>`;}).join("")}
