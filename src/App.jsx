  ...
  function buildClassResultsHTML(classStudents, className) {
    ...
    return `<div style="margin-bottom:32px;">
      ...
      <table class="results-table" style="width:100%;border-collapse:collapse;font-size:10px;min-width:600px;">
        <thead><tr style="background:rgba(59,7,100,0.85);">
          ...
          ${subs.map(s=>{const isComb=s===SCI_AGRI_COMBINED;const short=getSubShort(s);return`<th style="padding:4px 3px;text-align:center;max-width:40px;white-space:nowrap;vertical-align:bottom;background:${isComb?"rgba(13,148,136,0.85)":"rgba(59,7,100,0.85)"};"><div style="writing-mode:vertical-rl;text-orientation:mixed;transform:rotate(180deg);font-size:9px;font-weight:bold;line-height:1.2;height:90px;display:flex;align-items:flex-end;justify-content:center;color:white;" title="${s}">${short}</div></th>`;}).join("")}
        ...
        <tbody>
          ...
          <tr class="mean-row" style="background:rgba(245,243,255,0.7);font-weight:bold;">
            ...
            <td style="padding:5px 6px;"></td>
            ${subs.map(su=>{const isComb=su===SCI_AGRI_COMBINED;const allMarks=getColMarks(ranked,su);const m=allMarks.length?allMarks.reduce((a,b)=>a+b,0)/allMarks.length:0;return`<td style="padding:5px 4px;text-align:center;font-weight:bold;${isComb?`background:${SCI_AGRI_BG};color:${SCI_AGRI_DARK};`:""}">${m>0?m.toFixed(1):"—"}</td>`;}).join("")}
            <td style="padding:5px 6px;text-align:center;font-weight:bold;">${ranked.length?(ranked.map(s=>{const dm=subs.map(su=>getPrintMark(s.subs,su)).filter(v=>v!==null);return dm.length?dm.reduce((a,b)=>a+b,0)/dm.length:0;}).reduce((a,b)=>a+b,0)/ranked.length).toFixed(1):"—"}</td>
          </tr>
  ...
  function buildClassGradesHTML(classStudents, className) {
    ...
    const rows=ranked.map((s,i)=>{
      ...
      return `<tr style="background:${i%2===0?"rgba(255,255,255,0.55)":"rgba(248,250,252,0.55)"}">
        ...
        <td style="padding:5px 6px;text-align:center;">${og?`<span style="background:${og.bg};color:${og.col};font-size:8px;padding:2px 5px;border-radius:6px;font-weight:bold;">${dispAvg.toFixed(1)}/${og.g}</span>`:"—"}</td>
      </tr>`;
    }).join("");
    ...
    return `<div style="margin-bottom:32px;">
      ...
      <table class="grade-table" style="width:100%;border-collapse:collapse;font-size:10px;min-width:600px;">
        <thead><tr style="background:rgba(29,78,216,0.85);">
          <th style="padding:5px 6px;text-align:center;color:white;">Pos</th>
          <th style="padding:5px 6px;text-align:left;color:white;">Name</th>
          ${subs.map(s=>{const isComb=s===SCI_AGRI_COMBINED;const short=getSubShort(s);return`<th style="padding:4px 3px;text-align:center;max-width:40px;white-space:nowrap;vertical-align:bottom;background:${isComb?"rgba(13,148,136,0.85)":"rgba(29,78,216,0.85)"};"><div style="writing-mode:vertical-rl;text-orientation:mixed;transform:rotate(180deg);font-size:9px;font-weight:bold;line-height:1.2;height:90px;display:flex;align-items:flex-end;justify-content:center;color:white;" title="${s}">${short}</div></th>`;}).join("")}
