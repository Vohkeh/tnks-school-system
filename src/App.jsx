      ...
      <table class="results-table" style="width:100%;border-collapse:collapse;font-size:10px;min-width:600px;">
        ...
        <tbody>
          ...
          <tr class="mean-row" style="background:rgba(245,243,255,0.7);font-weight:bold;">
            ...
            <td style="padding:5px 6px;text-align:center;font-weight:bold;">${ranked.length?(ranked.map(s=>{const dm=subs.map(su=>getPrintMark(s.subs,su)).filter(v=>v!==null);return dm.length?dm.reduce((a,b)=>a+b,0)/dm.length:0;}).reduce((a,b)=>a+b,0)/ranked.length).toFixed(2):"—"}</td>
          </tr>
